...
const CATEGORIES = [
  { id: 'red-team', name: 'Red Team / Offensive Security', icon: 'target', description: 'Penetration testing, exploitation, social engineering, and attack simulation prompts.', count: 45, promptRange: [1, 45] },
  { id: 'blue-team', name: 'Blue Team / Defensive Security', icon: 'shield', description: 'Log analysis, incident response, defense strategy, and security architecture prompts.', count: 42, promptRange: [46, 87] },
  { id: 'soc-ops', name: 'SOC Operations', icon: 'monitor', description: 'SIEM queries, alert triage, shift operations, and SOC workflow prompts.', count: 42, promptRange: [88, 129] },
  { id: 'cloud-security', name: 'Cloud Security', icon: 'cloud', description: 'AWS, Azure, GCP, multi-cloud, container, and serverless security prompts.', count: 38, promptRange: [130, 167] },
  { id: 'osint', name: 'OSINT / Reconnaissance', icon: 'search', description: 'Domain intelligence, digital footprint, threat actor profiling, and recon automation.', count: 38, promptRange: [168, 205] },
  { id: 'grc', name: 'GRC / Compliance', icon: 'clipboard-check', description: 'Policy, risk assessment, audit, compliance frameworks, and governance prompts.', count: 38, promptRange: [206, 243] },
  { id: 'vuln-analysis', name: 'Vulnerability Analysis', icon: 'bug', description: 'CVE analysis, patch prioritization, vulnerability assessment, and reporting prompts.', count: 42, promptRange: [244, 285] },
  { id: 'ai-security', name: 'AI Agent Security', icon: 'bot', description: 'LLM testing, AI supply chain, agent security, and responsible AI governance.', count: 38, promptRange: [286, 323] },
];

const SKILL_LEVELS = {
  beginner: [1, 7, 46, 52, 88, 94, 130, 136, 168, 174, 206, 212, 244, 250, 286, 292],
  intermediate: [],
  advanced: []
};
(function(){
  const ranges = [[1, 15], [46, 59], [88, 101], [130, 141], [168, 179], [206, 217], [244, 257], [286, 297]];
  ranges.forEach(([s,e])=>{ for(let i=s;i<=e;i++) SKILL_LEVELS.intermediate.push(i); });
  const ranges2 = [[16, 45], [60, 87], [102, 129], [142, 167], [180, 205], [218, 243], [258, 285], [298, 323]];
  ranges2.forEach(([s,e])=>{ for(let i=s;i<=e;i++) SKILL_LEVELS.advanced.push(i); });
})();

function getSkillLevel(id) {
  if (SKILL_LEVELS.beginner.includes(id)) return 'Beginner';
  if (SKILL_LEVELS.intermediate.includes(id)) return 'Intermediate';
  return 'Advanced';
}

function getCategoryForPrompt(id) {
  if (id >= 1 && id <= 45) return 'red-team';
  if (id >= 46 && id <= 87) return 'blue-team';
  if (id >= 88 && id <= 129) return 'soc-ops';
  if (id >= 130 && id <= 167) return 'cloud-security';
  if (id >= 168 && id <= 205) return 'osint';
  if (id >= 206 && id <= 243) return 'grc';
  if (id >= 244 && id <= 285) return 'vuln-analysis';
  if (id >= 286 && id <= 323) return 'ai-security';
  return 'red-team';
}
...
const PROMPTS = [
  {
    ...
    category: "red-team",
    subcategory: "Reconnaissance & Enumeration",
    prompt: `I am conducting an authorized penetration test against [TARGET_DOMAIN]. Generate a comprehensive subdomain enumeration methodology that includes:
...
5. Priority ranking of discovered subdomains by likely attack surface value`,
    useWhen: "Starting a new engagement and need a structured recon plan.",
    variables: ["TARGET_DOMAIN"]
  },
  {
    id: 2,
    title: "Port Scan Result Analysis",
    category: "red-team",
    subcategory: "Reconnaissance & Enumeration",
    prompt: `Analyze the following Nmap scan results from an authorized pentest of [TARGET_IP]:
...
5. Recommend the order of attack based on risk-to-reward ratio`,
    useWhen: "You have raw Nmap output and need prioritized next steps.",
    variables: ["TARGET_IP", "PASTE_NMAP_OUTPUT"]
  },
  {
    id: 3,
    title: "DNS Zone Transfer Attempt Plan",
    category: "red-team",
    subcategory: "Reconnaissance & Enumeration",
    prompt: `I have authorization to test DNS configurations for [TARGET_DOMAIN]. Draft a step-by-step plan to:
...
5. Suggest fallback enumeration if zone transfer fails`,
    useWhen: "Early recon phase to map DNS infrastructure.",
    variables: ["TARGET_DOMAIN"]
  },
  {
    id: 4,
    title: "Web Application Attack Surface Mapping",
    category: "red-team",
    subcategory: "Reconnaissance & Enumeration",
    prompt: `I have discovered the following endpoints on [TARGET_WEB_APP] during an authorized assessment:
...
5. Recommend Burp Suite or ZAP configurations for automated testing`,
    useWhen: "You have a sitemap or crawl output and need to prioritize testing.",
    variables: ["TARGET_WEB_APP", "PASTE_ENDPOINT_LIST"]
  },
  {
    id: 5,
    title: "Network Service Fingerprinting Deep Dive",
    category: "red-team",
    subcategory: "Reconnaissance & Enumeration",
    prompt: `During an authorized engagement against [TARGET_NETWORK], I found the following services responding on non-standard ports:
...
5. Identify misconfigurations that could enable lateral movement`,
    useWhen: "You encounter services on unusual ports and need identification.",
    variables: ["TARGET_NETWORK", "SERVICE_LIST_WITH_PORTS"]
  },
  {
    id: 6,
    title: "OSINT-to-Attack-Path Correlation",
    category: "red-team",
    subcategory: "Reconnaissance & Enumeration",
    prompt: `From OSINT on [TARGET_ORGANIZATION], I have gathered the following:
- Employee names: [LIST]
- Email format: [FORMAT]
...
5. Rank each attack path by feasibility and likely impact`,
    useWhen: "Bridging OSINT findings into actionable attack scenarios.",
    variables: ["TARGET_ORGANIZATION", "LIST", "FORMAT", "TECH_STACK", "REPO_URLS", "PROFILE_LINKS"]
  },
  {
    id: 7,
    title: "SQL Injection Exploitation Roadmap",
    category: "red-team",
    subcategory: "Exploitation Methodology",
    prompt: `I have confirmed a SQL injection vulnerability at [URL_AND_PARAMETER] on [TARGET_APP] during an authorized test. The backend appears to be [DB_TYPE]. Create an exploitation roadmap:
...
6. Cleanup and evidence collection requirements`,
    useWhen: "You have a confirmed SQLi and need structured exploitation steps.",
    variables: ["URL_AND_PARAMETER", "TARGET_APP", "DB_TYPE"]
  },
  {
    id: 8,
    title: "Privilege Escalation Checklist Generator",
    category: "red-team",
    subcategory: "Exploitation Methodology",
    prompt: `I have a low-privilege shell on a [LINUX/WINDOWS] host during an authorized engagement. System details:
...
7. Ranked order of attempts from least noisy to most noisy`,
    useWhen: "Post-exploitation when you have initial access and need root/SYSTEM.",
    variables: ["LINUX/WINDOWS", "OS_VERSION", "USERNAME", "GROUP_MEMBERSHIP"]
  },
  {
    id: 9,
    title: "Active Directory Attack Path Planning",
    category: "red-team",
    subcategory: "Exploitation Methodology",
    prompt: `I have compromised a domain-joined workstation in [DOMAIN_NAME] with user [USERNAME]. BloodHound output shows:
...
6. Identify detection risks at each stage and evasion options`,
    useWhen: "Mid-engagement in an AD environment with BloodHound data.",
    variables: ["DOMAIN_NAME", "USERNAME", "PASTE_KEY_BLOODHOUND_FINDINGS"]
  },
  {
    id: 10,
    title: "Web Shell Deployment Strategy",
    category: "red-team",
    subcategory: "Exploitation Methodology",
    prompt: `During an authorized pentest, I have achieved [FILE_UPLOAD/RCE] on [TARGET_WEB_SERVER] running [WEB_SERVER_TYPE] with [LANGUAGE]. Design a web shell deployment strategy:
...
6. Note all forensic artifacts this will leave behind`,
    useWhen: "You need persistent access during an authorized engagement.",
    variables: ["FILE_UPLOAD/RCE", "TARGET_WEB_SERVER", "WEB_SERVER_TYPE", "LANGUAGE"]
  },
  {
    id: 11,
    title: "Buffer Overflow Exploit Development Guidance",
    category: "red-team",
    subcategory: "Exploitation Methodology",
    prompt: `I am developing an exploit for a buffer overflow vulnerability in [APPLICATION_NAME] version [VERSION] on [OS_PLATFORM]. I have:
- Crash offset: [OFFSET]
...
5. Adapting the exploit if ASLR/DEP is enabled`,
    useWhen: "Developing a custom exploit for a binary vulnerability.",
    variables: ["APPLICATION_NAME", "VERSION", "OS_PLATFORM", "OFFSET", "BAD_CHARS", "BYTES"]
  },
  {
    id: 12,
    ...
    category: "red-team",
    subcategory: "Exploitation Methodology",
    prompt: `I am authorized to test the REST API at [API_BASE_URL]. I have the following documentation:
...
6. Exact curl/Postman commands for each test case`,
    useWhen: "Scoping API penetration testing with documentation available.",
    variables: ["API_BASE_URL", "PASTE_API_DOCS_OR_SWAGGER_EXCERPT"]
  },
  {
    id: 13,
    title: "Phishing Campaign Template Design",
    category: "red-team",
    subcategory: "Social Engineering",
    prompt: `I am designing an authorized phishing simulation for [ORGANIZATION_NAME] targeting [DEPARTMENT]. Context:
- Industry: [INDUSTRY]
...
- Red flags that training should teach users to spot`,
    useWhen: "Planning authorized phishing simulations for security awareness.",
    variables: ["ORGANIZATION_NAME", "DEPARTMENT", "INDUSTRY", "RELEVANT_NEWS", "EMAIL_PLATFORM"]
  },
  {
    id: 14,
    title: "Vishing Script Development",
    category: "red-team",
    subcategory: "Social Engineering",
    prompt: `Create a vishing (voice phishing) script for an authorized social engineering engagement against [TARGET_ORGANIZATION]. Scenario: [PRETEXT_SCENARIO].
...
7. Ethical boundaries and abort triggers`,
    useWhen: "Preparing voice-based social engineering tests.",
    variables: ["TARGET_ORGANIZATION", "PRETEXT_SCENARIO"]
  },
  {
    id: 15,
    title: "Physical Social Engineering Pretext Planning",
    category: "red-team",
    subcategory: "Social Engineering",
    prompt: `Plan a physical social engineering assessment for [TARGET_FACILITY]. Known details:
...
For each: equipment needed, backstory, target areas, information objectives, abort criteria, and legal documentation to carry.`,
    useWhen: "Planning authorized physical penetration tests.",
    variables: ["TARGET_FACILITY", "BUILDING_TYPE", "OBSERVED_SECURITY", "DRESS_CODE", "OBSERVED_PATTERNS"]
  },
  {
    id: 16,
    title: "Social Engineering Report Narrative",
    category: "red-team",
    subcategory: "Social Engineering",
    prompt: `Write a professional social engineering assessment report narrative for the following engagement:
...
7. Training program recommendations`,
    useWhen: "Documenting social engineering engagement results.",
    variables: ["CLIENT_NAME", "PHISHING/VISHING/PHYSICAL", "DATE_RANGE", "KEY_METRICS", "FINDINGS_LIST"]
  },
  {
    id: 17,
    title: "Lateral Movement Strategy",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `I have compromised [INITIAL_HOST] in [NETWORK_SEGMENT] with [ACCESS_LEVEL] privileges during an authorized engagement. Network context:
...
7. Documentation requirements for each successful move`,
    useWhen: "Post-initial-access when you need to expand foothold.",
    variables: ["INITIAL_HOST", "NETWORK_SEGMENT", "ACCESS_LEVEL", "PASTE_NETWORK_MAP_OR_DISCOVERY_OUTPUT"]
  },
  {
    id: 18,
    title: "Data Exfiltration Simulation Plan",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `During an authorized red team engagement for [CLIENT], I need to simulate data exfiltration from [SOURCE_SYSTEM]. Security controls in place: [KNOWN_CONTROLS].
...
6. How the blue team should be able to detect each method`,
    useWhen: "Simulating data theft in red team exercises.",
    variables: ["CLIENT", "SOURCE_SYSTEM", "KNOWN_CONTROLS"]
  },
  {
    id: 19,
    title: "C2 Infrastructure Planning",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `I am setting up command-and-control infrastructure for an authorized red team engagement against [TARGET_ORG]. Their known security stack includes [SECURITY_TOOLS].
...
6. Infrastructure teardown checklist for engagement end`,
    useWhen: "Pre-engagement C2 infrastructure setup.",
    variables: ["TARGET_ORG", "SECURITY_TOOLS"]
  },
  {
    id: 20,
    title: "Persistence Mechanism Catalog",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `On a compromised [WINDOWS/LINUX/MACOS] system ([OS_VERSION]) during an authorized assessment, I need persistence that survives reboots. Current access level: [ACCESS_LEVEL].
...
6. Recommended mechanism based on the engagement's stealth requirements`,
    useWhen: "Establishing persistence during red team operations.",
    variables: ["WINDOWS/LINUX/MACOS", "OS_VERSION", "ACCESS_LEVEL"]
  },
  {
    id: 21,
    title: "Password Spraying Campaign Design",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `I am authorized to conduct password spraying against [TARGET_DOMAIN] with the following known information:
...
6. Logging requirements for the engagement report`,
    useWhen: "Planning credential spraying within safe limits.",
    variables: ["TARGET_DOMAIN", "ENDPOINT", "NUMBER", "FORMAT", "POLICY_DETAILS", "LOCKOUT_THRESHOLD_AND_DURATION"]
  },
  {
    id: 22,
    title: "Wireless Network Attack Plan",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `I am authorized to assess wireless security at [TARGET_LOCATION]. Observed networks:
...
6. Physical positioning recommendations for optimal results`,
    useWhen: "Conducting authorized wireless security assessments.",
    variables: ["TARGET_LOCATION", "PASTE_AIRODUMP_OR_SURVEY_OUTPUT"]
  },
  {
    id: 23,
    title: "Red Team Engagement Rules of Engagement Draft",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `Draft a Rules of Engagement (RoE) document for a red team assessment with these parameters:
...
7. Deliverables and timeline`,
    useWhen: "Formalizing engagement scope before a red team exercise.",
    variables: ["CLIENT_NAME", "IN_SCOPE_SYSTEMS_AND_NETWORKS", "START_DATE", "END_DATE", "ASSUMED_BREACH/FULL_SCOPE/TARGETED", "EXCLUDED_SYSTEMS", "CONTACT_LIST"]
  },
  {
    id: 24,
    ...
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `I have a valid domain credential ([USERNAME]) in [DOMAIN_NAME] during an authorized assessment. I want to perform Kerberoasting.
...
6. Detection indicators the blue team should look for`,
    useWhen: "Targeting service account credentials in Active Directory.",
    variables: ["USERNAME", "DOMAIN_NAME"]
  },
  {
    id: 25,
    title: "Red Team Debrief Report Structure",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `Structure a red team debrief report for [ENGAGEMENT_NAME]:
...
7. Technical appendix with tool output and commands used`,
    useWhen: "Writing the final deliverable after a red team engagement.",
    variables: ["ENGAGEMENT_NAME", "LIST_KEY_FINDINGS"]
  },
  {
    id: 26,
    ...
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `During an authorized assessment of [TARGET_APPLICATION], I have identified these individual vulnerabilities:
...
6. Write the chain narrative for the pentest report`,
    useWhen: "Multiple low/medium findings that could chain into critical impact.",
    variables: ["TARGET_APPLICATION", "LIST_VULNERABILITIES_WITH_DETAILS"]
  },
  {
    id: 27,
    title: "Client-Side Attack Payload Generation",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `For an authorized engagement, I need a client-side payload targeting [TARGET_OS] with [TARGET_APPLICATION]. The target's AV/EDR is [SECURITY_PRODUCT].
...
6. Sandbox evasion techniques to include`,
    useWhen: "Crafting client-side attacks for social engineering delivery.",
    variables: ["TARGET_OS", "TARGET_APPLICATION", "SECURITY_PRODUCT"]
  },
  {
    id: 28,
    title: "Container Escape Methodology",
    category: "red-team",
    subcategory: "Attack Path & Post-Exploitation",
    prompt: `I have access to a container on [CONTAINER_RUNTIME] (Docker/Podman/containerd) during an authorized assessment. Container details:
...
7. Post-escape pivoting steps`,
    useWhen: "Testing container isolation during cloud/infrastructure pentests.",
    variables: ["CONTAINER_RUNTIME"]
  },
  {
    id: 29,
    title: "Custom Payload Obfuscation",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I have a [LANGUAGE] payload that is detected by [AV/EDR_PRODUCT]. The payload:
...
6. Testing methodology to validate evasion`,
    useWhen: "Bypassing specific security products during authorized testing.",
    variables: ["LANGUAGE", "AV/EDR_PRODUCT", "PASTE_PAYLOAD_OR_DESCRIBE_FUNCTIONALITY"]
  },
  {
    id: 30,
    ...
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `During an authorized internal assessment, I've identified potential NTLM relay targets. Network details:
...
6. Cleanup steps after successful relay`,
    useWhen: "Exploiting NTLM relay vulnerabilities in Windows environments.",
    variables: ["SIGNING_STATUS_PER_HOST", "SERVICE_LIST", "ATTACKER_IP_AND_SEGMENT"]
  },
  {
    id: 31,
    title: "Cloud Penetration Test Scope Builder",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I am planning a cloud penetration test for [ORGANIZATION] on [AWS/AZURE/GCP]. Known cloud footprint:
...
6. Create the cloud-specific Rules of Engagement addendum`,
    useWhen: "Scoping cloud-specific penetration testing engagements.",
    variables: ["ORGANIZATION", "AWS/AZURE/GCP", "PASTE_KNOWN_ASSETS"]
  },
  {
    id: 32,
    title: "Purple Team Exercise Scenario Design",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `Design a purple team exercise for [ORGANIZATION] testing detection of [MITRE_TECHNIQUE_IDS]. Environment:
- SIEM: [SIEM_PLATFORM]
- EDR: [EDR_PRODUCT]
...
6. Tuning recommendations for the detection rule`,
    useWhen: "Running collaborative red/blue team detection improvement exercises.",
    variables: ["ORGANIZATION", "MITRE_TECHNIQUE_IDS", "SIEM_PLATFORM", "EDR_PRODUCT", "NSM_TOOLS"]
  },
  {
    id: 33,
    title: "Assumed Breach Scenario Planning",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `Plan an assumed breach scenario for [TARGET_ORG] starting from:
...
6. Daily deconfliction and status reporting schedule`,
    useWhen: "Planning assumed breach red team exercises.",
    variables: ["TARGET_ORG", "COMPROMISED_ASSET", "ACCESS_LEVEL", "LIST_CROWN_JEWELS", "DURATION"]
  },
  {
    id: 34,
    title: "Thick Client Application Testing Guide",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I am authorized to test a thick client application: [APP_NAME] running on [OS]. Technology stack: [TECH_STACK].
...
7. Recommended tools for each testing phase`,
    useWhen: "Assessing desktop/thick client applications.",
    variables: ["APP_NAME", "OS", "TECH_STACK"]
  },
  {
    id: 35,
    title: "SCADA/ICS Penetration Testing Framework",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I am authorized to test an ICS/SCADA environment for [ORGANIZATION]. System details:
...
7. ICS-specific reporting requirements`,
    useWhen: "Testing operational technology environments with extreme caution.",
    variables: ["ORGANIZATION", "MODBUS/DNP3/OPC/ETC", "DEVICE_LIST", "ARCHITECTURE_OVERVIEW"]
  },
  {
    id: 36,
    title: "Mobile Application Penetration Testing Checklist",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I am authorized to test the mobile application [APP_NAME] on [ANDROID/IOS]. The app communicates with [BACKEND_URL].
...
7. Exact tool commands (objection, frida, jadx, apktool)`,
    useWhen: "Conducting mobile application security assessments.",
    variables: ["APP_NAME", "ANDROID/IOS", "BACKEND_URL"]
  },
  {
    id: 37,
    title: "Ransomware Simulation Design",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `Design a safe ransomware simulation for [ORGANIZATION]'s tabletop or controlled technical exercise. Purpose: testing backup/recovery and incident response.
...
8. Explicit exclusions to prevent any actual damage`,
    useWhen: "Testing ransomware resilience without real risk.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 38,
    title: "Bypassing MFA Assessment",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `During an authorized assessment, I need to test MFA implementation on [TARGET_APPLICATION] using [MFA_TYPE] (TOTP, SMS, push notification, FIDO2).
...
7. Impact assessment for each successful bypass`,
    useWhen: "Validating MFA implementations during web application assessments.",
    variables: ["TARGET_APPLICATION", "MFA_TYPE"]
  },
  {
    id: 39,
    title: "Internal Network Segmentation Testing",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I am authorized to validate network segmentation for [ORGANIZATION]. Provided access in [ZONE_A], testing connectivity to [ZONE_B, ZONE_C, etc.].
...
7. Recommendations for segmentation improvements`,
    useWhen: "Validating firewall rules and network segmentation controls.",
    variables: ["ORGANIZATION", "ZONE_A"]
  },
  {
    id: 40,
    title: "Post-Exploitation Evidence Collection",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I have completed exploitation phases in an authorized engagement. Compromised hosts:
...
7. Secure evidence storage and transfer procedures`,
    useWhen: "Preparing engagement evidence for the final report.",
    variables: ["HOST_LIST_WITH_ACCESS_LEVELS"]
  },
  {
    id: 41,
    title: "Attack Surface Reduction Recommendations",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `Based on the following pentest findings for [ORGANIZATION]:
...
6. Metrics to track improvement over time`,
    useWhen: "Translating pentest findings into a remediation roadmap.",
    variables: ["ORGANIZATION", "PASTE_FINDINGS_SUMMARY"]
  },
  {
    id: 42,
    title: "EDR Evasion Research Plan",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I need to test whether [EDR_PRODUCT] detects [SPECIFIC_TECHNIQUE] during an authorized red team exercise. The EDR version is [VERSION].
...
6. Document findings for blue team detection improvement`,
    useWhen: "Researching detection gaps in endpoint security products.",
    variables: ["EDR_PRODUCT", "SPECIFIC_TECHNIQUE", "VERSION"]
  },
  {
    id: 43,
    ...
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `Design a supply chain attack simulation for [ORGANIZATION]'s authorized security exercise:
...
6. Realistic but safe proof-of-concept for each vector`,
    useWhen: "Testing software supply chain security posture.",
    variables: ["ORGANIZATION", "KEY_DEPENDENCIES", "CI_CD_PLATFORM", "PACKAGE_MANAGERS"]
  },
  {
    id: 44,
    title: "Cobalt Strike Malleable C2 Profile Review",
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `Review and improve this Cobalt Strike malleable C2 profile for an authorized red team engagement against [TARGET_ORG]:
...
7. Test the profile against common detection tools`,
    useWhen: "Hardening C2 communications before red team engagements.",
    variables: ["TARGET_ORG", "PASTE_PROFILE"]
  },
  {
    id: 45,
    ...
    category: "red-team",
    subcategory: "Advanced Techniques",
    prompt: `I am researching [APPLICATION_NAME] version [VERSION] for potential zero-day vulnerabilities during an authorized research program. Source access: [YES/NO].
...
6. Responsible disclosure timeline and coordination plan`,
    useWhen: "Conducting authorized vulnerability research on specific applications.",
    variables: ["APPLICATION_NAME", "VERSION", "YES/NO"]
  },
  {
    id: 46,
    title: "Suspicious Authentication Log Analysis",
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `Analyze the following authentication log entries from [LOG_SOURCE] for indicators of compromise:
...
7. Provide SIEM query to find similar activity across all sources`,
    useWhen: "Reviewing authentication logs during threat hunting or alert triage.",
    variables: ["LOG_SOURCE", "PASTE_LOG_ENTRIES"]
  },
  {
    id: 47,
    title: "PowerShell Script Block Log Investigation",
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `I've collected the following PowerShell Script Block log entries (Event ID 4104) from [HOST_NAME]:
...
7. Recommend containment steps based on findings`,
    useWhen: "Investigating potential PowerShell-based attacks.",
    variables: ["HOST_NAME", "PASTE_LOG_ENTRIES"]
  },
  {
    id: 48,
    title: "Threat Hunting Hypothesis Generator",
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `Generate threat hunting hypotheses for [ORGANIZATION] based on:
- Industry: [INDUSTRY]
...
6. Escalation criteria if findings are suspicious`,
    useWhen: "Planning proactive threat hunting campaigns.",
    variables: ["ORGANIZATION", "INDUSTRY", "RELEVANT_THREATS_OR_CAMPAIGNS", "LIST_OF_LOG_SOURCES", "TECHNIQUE_IDS"]
  },
  {
    id: 49,
    title: "DNS Log Anomaly Detection",
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `Analyze these DNS query logs from [DNS_SERVER] for signs of malicious activity:
...
7. Provide detection rules for each identified pattern`,
    useWhen: "Hunting for DNS-based C2 or data exfiltration.",
    variables: ["DNS_SERVER", "PASTE_DNS_LOGS"]
  },
  {
    id: 50,
    title: "Windows Event Log Forensic Analysis",
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `I have the following Windows Event Logs from a potentially compromised host [HOST_NAME]:
...
7. IOCs extracted for organizational-wide sweep`,
    useWhen: "Conducting forensic analysis on a suspected compromised Windows host.",
    variables: ["HOST_NAME", "PASTE_EVENTS"]
  },
  {
    id: 51,
    ...
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `Our network monitoring has flagged anomalous traffic from [SOURCE_IP] to [DESTINATION_IP]. PCAP analysis summary:
...
7. Verdict: benign, suspicious, or malicious with confidence rating`,
    useWhen: "Investigating network-based alerts or anomalies.",
    variables: ["SOURCE_IP", "DESTINATION_IP", "PASTE_TRAFFIC_SUMMARY_OR_ZEEK_LOGS"]
  },
  {
    id: 52,
    title: "Email Header Analysis for Phishing Investigation",
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `Analyze these email headers from a suspected phishing email reported by [USER]:
...
8. Recommend organizational-wide search for similar messages`,
    useWhen: "Investigating reported phishing emails.",
    variables: ["USER", "PASTE_FULL_EMAIL_HEADERS"]
  },
  {
    id: 53,
    title: "Threat Hunting Playbook: Persistence Mechanisms",
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `Create a threat hunting playbook for detecting persistence mechanisms across our [WINDOWS/LINUX/MIXED] environment.
...
6. Response actions for confirmed malicious persistence`,
    useWhen: "Building systematic persistence detection capabilities.",
    variables: ["WINDOWS/LINUX/MIXED", "LIST_SOURCES", "SIEM_PLATFORM"]
  },
  {
    id: 54,
    ...
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `I have a suspicious file [FILENAME] with hash [FILE_HASH] collected from [SOURCE_HOST]. File type: [FILE_TYPE].
...
7. Containment steps based on preliminary findings`,
    useWhen: "First encounter with a suspicious file that needs rapid assessment.",
    variables: ["FILENAME", "FILE_HASH", "SOURCE_HOST", "FILE_TYPE"]
  },
  {
    id: 55,
    title: "Linux Intrusion Detection Log Review",
    category: "blue-team",
    subcategory: "Log Analysis & Threat Hunting",
    prompt: `Review these logs from a Linux server [HOSTNAME] suspected of compromise:
...
7. Timeline of attacker activity with confidence ratings`,
    useWhen: "Investigating suspected Linux server compromises.",
    variables: ["HOSTNAME", "PASTE_RELEVANT_ENTRIES", "PASTE_IF_AVAILABLE"]
  },
  {
    id: 56,
    ...
    prompt: `Create a ransomware incident response playbook for [ORGANIZATION]:
...
8. Post-incident review agenda`,
    useWhen: "Building or updating ransomware response procedures.",
    variables: ["ORGANIZATION", "COUNT", "OS_TYPES", "BACKUP_SYSTEM", "EDR_PRODUCT", "BRIEF_DESCRIPTION"]
  },
  {
    id: 57,
    title: "Business Email Compromise Response Plan",
    category: "blue-team",
    subcategory: "Incident Response",
    prompt: `Create a BEC incident response plan. Scenario: [DESCRIBE_BEC_SCENARIO]. Known details:
...
8. Preventive controls to implement post-incident`,
    useWhen: "Responding to business email compromise incidents.",
    variables: ["DESCRIBE_BEC_SCENARIO", "ACCOUNT", "TIMELINE", "AMOUNT_OR_UNKNOWN", "EMAIL_PLATFORM"]
  },
  {
    id: 58,
    ...
    category: "blue-team",
    subcategory: "Incident Response",
    prompt: `Create an incident severity classification matrix for [ORGANIZATION] with:
...
7. Reclassification triggers (when to upgrade/downgrade)`,
    useWhen: "Establishing or refining incident classification standards.",
    variables: ["ORGANIZATION", "LIST_CRITICAL_ASSETS", "LIST_SERVICES", "COMPLIANCE_FRAMEWORKS"]
  },
  {
    id: 59,
    title: "Containment Strategy Decision Tree",
    category: "blue-team",
    subcategory: "Incident Response",
    prompt: `Build a containment strategy decision tree for [INCIDENT_TYPE] affecting [SYSTEM_TYPE] in our environment.
...
7. Criteria for moving from containment to eradication`,
    useWhen: "Making real-time containment decisions during active incidents.",
    variables: ["INCIDENT_TYPE", "SYSTEM_TYPE", "HIGH/MEDIUM/LOW", "YES/NO/SCHEDULED_ONLY", "REQUIREMENTS"]
  },
  {
    id: 60,
    title: "Incident Communication Template Suite",
    category: "blue-team",
    subcategory: "Incident Response",
    prompt: `Create a suite of incident communication templates for [ORGANIZATION] for a [INCIDENT_TYPE] incident:
...
For each template, include placeholders for [TIMESTAMP], [IMPACT], [ACTIONS_TAKEN], [NEXT_STEPS].`,
    useWhen: "Preparing communication templates before or during incidents.",
    variables: ["ORGANIZATION", "INCIDENT_TYPE", "REGULATORY_BODY", "TIMESTAMP", "IMPACT", "ACTIONS_TAKEN", "NEXT_STEPS"]
  },
  {
    id: 61,
    title: "Defense-in-Depth Architecture Review",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Review our defense-in-depth architecture for gaps:
...
6. Priority investments ranked by risk reduction`,
    useWhen: "Annual security architecture review or after a significant incident.",
    variables: []
  },
  {
    id: 62,
    title: "Detection Engineering Rule Development",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Write detection rules for [MITRE_TECHNIQUE_ID] - [TECHNIQUE_NAME] for our environment:
- SIEM: [SIEM_PLATFORM]
- EDR: [EDR_PRODUCT]
...
8. Known bypass techniques for this detection`,
    useWhen: "Building new detection capabilities for specific threats.",
    variables: ["MITRE_TECHNIQUE_ID", "TECHNIQUE_NAME", "SIEM_PLATFORM", "EDR_PRODUCT", "LOG_SOURCES"]
  },
  {
    id: 63,
    title: "Firewall Rule Audit Checklist",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Audit the following firewall rules for [FIREWALL_TYPE] at [NETWORK_BOUNDARY]:
...
8. Recommended consolidation and cleanup actions`,
    useWhen: "Periodic firewall rule hygiene reviews.",
    variables: ["FIREWALL_TYPE", "NETWORK_BOUNDARY", "PASTE_FIREWALL_RULES_OR_SUMMARY"]
  },
  {
    id: 64,
    title: "Endpoint Hardening Baseline Generator",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Generate an endpoint hardening baseline for [WINDOWS_VERSION/LINUX_DISTRO] in a [CORPORATE/SERVER/DEVELOPER] role:
...
9. Validation script outline to verify compliance`,
    useWhen: "Creating or updating endpoint hardening standards.",
    variables: ["WINDOWS_VERSION/LINUX_DISTRO", "CORPORATE/SERVER/DEVELOPER", "CIS/STIG/CUSTOM"]
  },
  {
    id: 65,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Design a network segmentation strategy for [ORGANIZATION]:
...
8. Validation testing methodology`,
    useWhen: "Designing or improving network segmentation architecture.",
    variables: ["ORGANIZATION", "DESCRIPTION", "LIST_WITH_LOCATIONS", "DEPARTMENTS/ROLES", "FRAMEWORKS"]
  },
  {
    id: 66,
    title: "Security Monitoring Gap Analysis",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Perform a monitoring gap analysis for [ORGANIZATION]:
...
8. 12-month monitoring improvement roadmap`,
    useWhen: "Assessing completeness of security monitoring coverage.",
    variables: ["ORGANIZATION", "LIST_WHAT_IS_CURRENTLY_LOGGED_AND_WHERE"]
  },
  {
    id: 67,
    title: "Deception Technology Deployment Plan",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Design a deception/honeypot deployment plan for [ORGANIZATION]:
...
8. Integration with SIEM and incident response workflow`,
    useWhen: "Adding deception-based detection to the security architecture.",
    variables: ["ORGANIZATION", "LIST", "SERVICES_IN_USE", "TTPS"]
  },
  {
    id: 68,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Create security awareness training content for [AUDIENCE: all employees / IT staff / executives / developers] at [ORGANIZATION]:
...
7. Customization hooks for industry-specific threats`,
    useWhen: "Building or refreshing security awareness programs.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 69,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Assess [ORGANIZATION]'s readiness for Zero Trust Architecture:
...
7. 18-month Zero Trust roadmap with milestones`,
    useWhen: "Planning or assessing Zero Trust transformation initiatives.",
    variables: ["ORGANIZATION", "IAM_SOLUTION", "CURRENT_STATE", "MDM/UEM", "VPN/ZTNA/DIRECT", "STATUS"]
  },
  {
    id: 70,
    title: "Email Security Gateway Configuration Review",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Review our email security configuration for [EMAIL_SECURITY_PRODUCT]:
...
8. Specific recommendations with configuration changes`,
    useWhen: "Auditing email security posture.",
    variables: ["EMAIL_SECURITY_PRODUCT", "PASTE_CONFIGURATION_SUMMARY"]
  },
  {
    id: 71,
    title: "SIEM Use Case Development Roadmap",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Develop a SIEM use case roadmap for [SIEM_PLATFORM] at [ORGANIZATION]:
...
7. Retirement criteria for underperforming use cases`,
    useWhen: "Building a structured SIEM detection roadmap.",
    variables: ["SIEM_PLATFORM", "ORGANIZATION", "LIST_EXISTING", "LIST_SOURCES", "THREAT_PROFILE"]
  },
  {
    id: 72,
    title: "Backup and Recovery Security Review",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Assess the security of backup and recovery infrastructure for [ORGANIZATION]:
...
8. Specific recommendations for ransomware resilience`,
    useWhen: "Validating backup security as part of ransomware readiness.",
    variables: ["ORGANIZATION", "BACKUP_PRODUCT", "FREQUENCY", "POLICY"]
  },
  {
    id: 73,
    title: "Threat Intelligence Integration Plan",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Create a threat intelligence integration plan for [ORGANIZATION]:
...
7. Information sharing participation plan (ISAC, trusted circles)`,
    useWhen: "Building or maturing a threat intelligence program.",
    variables: ["ORGANIZATION", "EXISTING_FEEDS", "INDUSTRY_AND_LIKELY_ADVERSARIES"]
  },
  {
    id: 74,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Design a vulnerability management program for [ORGANIZATION]:
...
8. Executive reporting template`,
    useWhen: "Establishing or overhauling vulnerability management processes.",
    variables: ["ORGANIZATION", "COUNT", "TOOL_AND_FREQUENCY", "CURRENT_PROCESS"]
  },
  {
    id: 75,
    title: "Security Tool Consolidation Analysis",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Analyze our security tool stack for consolidation opportunities:
...
8. Migration plan for recommended consolidation`,
    useWhen: "Rationalizing the security tool portfolio during budget reviews.",
    variables: ["LIST_ALL_SECURITY_TOOLS_WITH_CATEGORY_AND_ANNUAL_COST"]
  },
  {
    id: 76,
    title: "DNS Security Hardening Guide",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Create a DNS security hardening guide for [ORGANIZATION]:
...
8. Registrar security (lock, MFA, notification alerts)`,
    useWhen: "Strengthening DNS infrastructure security.",
    variables: ["ORGANIZATION", "SERVERS/SERVICES", "HOSTING_PROVIDER", "CURRENT_CONFIG"]
  },
  {
    id: 77,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Design an insider threat detection program for [ORGANIZATION]:
...
8. Deterrence controls (banners, training, access reviews)`,
    useWhen: "Establishing insider threat detection capabilities.",
    variables: ["ORGANIZATION", "INDUSTRY", "COUNT", "LIST_SENSITIVE_DATA", "DLP_TOOLS", "SIEM/UBA_PLATFORM"]
  },
  {
    id: 78,
    title: "Patch Management Policy Draft",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Draft a patch management policy for [ORGANIZATION]:
...
8. Roles and responsibilities`,
    useWhen: "Creating or updating organizational patch management policies.",
    variables: ["ORGANIZATION", "LIST", "TOOL", "OVERVIEW", "FRAMEWORKS"]
  },
  {
    id: 79,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Create a security architecture review checklist for [PROJECT/APPLICATION]:
...
9. Threat model validation points`,
    useWhen: "Reviewing security architecture for new projects or major changes.",
    variables: ["PROJECT/APPLICATION", "MONOLITH/MICROSERVICES/SERVERLESS", "LEVELS", "EXTERNAL_SYSTEMS", "INTERNAL/EXTERNAL/BOTH"]
  },
  {
    id: 80,
    title: "Incident Lessons Learned Facilitator Guide",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Create a facilitation guide for an incident lessons learned / post-mortem session:
...
8. Follow-up tracking process`,
    useWhen: "Conducting post-incident review sessions.",
    variables: ["BRIEF_DESCRIPTION", "TIME_FROM_DETECTION_TO_RESOLUTION", "LIST_TEAMS"]
  },
  {
    id: 81,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Design a security metrics dashboard for [AUDIENCE: CISO / SOC Manager / Board]:
...
8. Monthly narrative template to accompany the dashboard`,
    useWhen: "Building security reporting dashboards for different stakeholders.",
    variables: []
  },
  {
    id: 82,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Create an Active Directory security hardening plan for [DOMAIN_NAME]:
...
9. Legacy protocol deprecation plan (NTLMv1, LLMNR, NetBIOS)`,
    useWhen: "Hardening Active Directory environments against modern attack techniques.",
    variables: ["DOMAIN_NAME", "LEVEL", "COUNT", "SUMMARY", "LIST"]
  },
  {
    id: 83,
    title: "Cloud-to-On-Prem Hybrid Security Architecture",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Design security controls for the hybrid connectivity between [CLOUD_PROVIDER] and our on-premises environment:
...
8. Break-glass access procedures for both environments`,
    useWhen: "Securing hybrid cloud architectures.",
    variables: ["CLOUD_PROVIDER", "VPN/EXPRESSROUTE/DIRECT_CONNECT", "LIST", "AD_SYNC/FEDERATION"]
  },
  {
    id: 84,
    title: "Security Awareness Phishing Metrics Analysis",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Analyze our phishing simulation results and recommend improvements:
...
8. KPIs to track over the next 12 months`,
    useWhen: "Evaluating and improving security awareness programs.",
    variables: ["COUNT", "PERCENTAGE", "DATA", "PREVIOUS_METRICS"]
  },
  {
    id: 85,
    title: "Application Allowlisting Policy",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Design an application allowlisting policy for [ORGANIZATION]:
...
8. Monitoring and incident handling for blocked applications`,
    useWhen: "Implementing application control policies.",
    variables: ["ORGANIZATION", "WORKSTATIONS/SERVERS/BOTH", "APPLOCKER/WDAC/THIRD_PARTY", "STANDARD/DEVELOPER/ADMIN"]
  },
  {
    id: 86,
    ...
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Design a secure remote access architecture for [ORGANIZATION]:
...
8. Fallback access methods for emergencies`,
    useWhen: "Designing or modernizing remote access infrastructure.",
    variables: ["ORGANIZATION", "COUNT", "MANAGED/BYOD/BOTH", "LIST", "FRAMEWORKS"]
  },
  {
    id: 87,
    title: "Security Operations Budget Justification",
    category: "blue-team",
    subcategory: "Defense Strategy",
    prompt: `Help me build a budget justification for security operations improvements:
...
8. Success metrics to demonstrate value post-investment`,
    useWhen: "Preparing security budget proposals for leadership approval.",
    variables: ["LIST_PROPOSED_TOOLS_OR_HEADCOUNT_WITH_COSTS", "LIST_CURRENT_GAPS", "LIST_INCIDENTS_AND_COSTS"]
  },
  {
    id: 88,
    title: "Splunk Query: Brute Force Detection",
    category: "soc-ops",
    subcategory: "SIEM Query Generation",
    prompt: `Write a Splunk SPL query to detect brute force attacks against [AUTHENTICATION_SYSTEM]:
...
8. Provide the equivalent saved search configuration`,
    useWhen: "Building brute force detection in Splunk.",
    variables: ["AUTHENTICATION_SYSTEM", "INDEX_AND_SOURCETYPE", "LIST_FIELDS", "LIST"]
  },
  {
    id: 89,
    title: "Microsoft Sentinel KQL: Suspicious Process Execution",
    category: "soc-ops",
    subcategory: "SIEM Query Generation",
    prompt: `Write a Microsoft Sentinel KQL query to detect suspicious process execution chains:
...
8. Provide test instructions to validate detection`,
    useWhen: "Creating detection analytics in Microsoft Sentinel.",
    variables: ["DESCRIBE_SPECIFIC_BEHAVIOR", "KNOWN_LEGIT"]
  },
  {
    id: 90,
    title: "Elastic SIEM Query: Data Exfiltration",
    category: "soc-ops",
    subcategory: "SIEM Query Generation",
    prompt: `Write Elasticsearch/Kibana queries for detecting potential data exfiltration:
...
8. Create a Kibana dashboard layout for visualization`,
    useWhen: "Building data exfiltration detection in Elastic SIEM.",
    variables: ["PATTERNS", "THRESHOLD", "TIMEFRAME"]
  },
  {
    id: 91,
    title: "SIEM Query: Lateral Movement Detection",
    category: "soc-ops",
    subcategory: "SIEM Query Generation",
    prompt: `Write [SIEM_PLATFORM] queries to detect lateral movement in a Windows domain:
...
8. For each query: false positive tuning guidance and analyst investigation steps`,
    useWhen: "Detecting lateral movement techniques in Windows environments.",
    variables: ["SIEM_PLATFORM", "LIST"]
  },
  {
    id: 92,
    title: "SIEM Query: Privilege Escalation Detection",
    category: "soc-ops",
    subcategory: "SIEM Query Generation",
    prompt: `Write [SIEM_PLATFORM] queries to detect privilege escalation attempts:
...
8. Provide severity ratings and investigation playbook links for each`,
    useWhen: "Building privilege escalation detection capabilities.",
    variables: ["SIEM_PLATFORM", "WINDOWS/LINUX/BOTH", "LIST"]
  },
  {
    id: 93,
    title: "SIEM Correlation Rule: Multi-Stage Attack",
    category: "soc-ops",
    subcategory: "SIEM Query Generation",
    prompt: `Design a multi-stage SIEM correlation rule for [SIEM_PLATFORM] that links:
...
8. Provide the correlation rule in the SIEM's native format`,
    useWhen: "Detecting complex multi-stage attacks through event correlation.",
    variables: ["SIEM_PLATFORM", "INITIAL_ACCESS_INDICATOR", "EXECUTION_INDICATOR", "PERSISTENCE_OR_LATERAL_MOVEMENT_INDICATOR", "TIMEFRAME", "HOST/USER/IP"]
  },
  {
    id: 94,
    ...
    category: "soc-ops",
    subcategory: "SIEM Query Generation",
    prompt: `Create a log source onboarding checklist for adding [LOG_SOURCE_TYPE] to [SIEM_PLATFORM]:
...
8. Documentation: source documentation template for SOC reference`,
    useWhen: "Adding new log sources to the SIEM in a structured way.",
    variables: ["LOG_SOURCE_TYPE", "SIEM_PLATFORM", "SYSTEM_NAME", "SYSLOG/JSON/CEF/OTHER", "EPS_ESTIMATE", "SYSLOG_TCP/API/AGENT/FILE"]
  },
  {
    id: 95,
    title: "Alert Triage Decision Framework",
    category: "soc-ops",
    subcategory: "Alert Analysis & Response",
    prompt: `Create an alert triage decision framework for SOC analysts handling [ALERT_TYPE] from [SECURITY_TOOL]:
...
8. Response time targets for each step`,
    useWhen: "Standardizing alert triage for specific alert types.",
    variables: ["ALERT_TYPE", "SECURITY_TOOL", "PASTE_SAMPLE_ALERT"]
  },
  {
    id: 96,
    title: "SOC Analyst Investigation Notebook",
    category: "soc-ops",
    subcategory: "Alert Analysis & Response",
    prompt: `Create an investigation notebook template for a [INCIDENT_TYPE] investigation:
...
10. Final disposition and lessons learned`,
    useWhen: "Structuring SOC investigations for consistency and completeness.",
    variables: ["INCIDENT_TYPE", "PASTE_ALERT_DETAILS"]
  },
  {
    id: 97,
    title: "False Positive Reduction Analysis",
    category: "soc-ops",
    subcategory: "Alert Analysis & Response",
    prompt: `Analyze false positive patterns for [ALERT_RULE_NAME] in our SOC:
...
8. Monitoring plan to verify tuning effectiveness`,
    useWhen: "Reducing alert fatigue by tuning noisy detection rules.",
    variables: ["ALERT_RULE_NAME", "COUNT", "PERCENTAGE", "LIST_IF_KNOWN"]
  },
  {
    id: 98,
    title: "SOC Runbook: Malware Alert Response",
    category: "soc-ops",
    subcategory: "Alert Analysis & Response",
    prompt: `Create a SOC runbook for responding to malware detection alerts from [EDR/AV_PRODUCT]:
...
8. Ticket documentation requirements and SLA tracking`,
    useWhen: "Standardizing malware alert response for SOC analysts.",
    variables: ["EDR/AV_PRODUCT", "MALWARE_DETECTION/BEHAVIORAL_ALERT/FILE_QUARANTINE", "THREAT_INTEL_SOURCES"]
  },
  {
    id: 99,
    title: "SOC Runbook: Phishing Email Response",
    category: "soc-ops",
    subcategory: "Alert Analysis & Response",
    prompt: `Create a SOC runbook for user-reported phishing emails:
...
10. Metrics: track volume, response time, user reporting rate`,
    useWhen: "Building phishing response procedures for the SOC.",
    variables: ["PLATFORM", "SYSTEM", "SLA"]
  },
  {
    id: 100,
    title: "Automated Triage Enrichment Workflow",
    category: "soc-ops",
    subcategory: "Alert Analysis & Response",
    prompt: `Design an automated alert enrichment workflow for [SOAR_PLATFORM]:
...
10. Escalation criteria (when to page the on-call analyst)`,
    useWhen: "Building SOAR playbooks for automated alert enrichment.",
    variables: ["SOAR_PLATFORM", "SECURITY_TOOL", "THRESHOLD", "THREAT_INTEL_APIS_TO_USE", "SERVICES", "CMDB/ASSET_DB", "DIRECTORY_SERVICE", "VM_SCANNER"]
  },
  {
    id: 101,
    title: "Alert Correlation Scenario Design",
    category: "soc-ops",
    subcategory: "Alert Analysis & Response",
    prompt: `Design alert correlation scenarios for [SIEM_PLATFORM] to detect [ATTACK_SCENARIO]:
...
9. Suppression rules to prevent duplicate correlations`,
    useWhen: "Creating sophisticated multi-source alert correlation.",
    variables: ["SIEM_PLATFORM", "ATTACK_SCENARIO", "LIST_SECURITY_TOOLS_AND_ALERT_TYPES"]
  },
  {
    id: 102,
    title: "SOC Shift Handoff Report Template",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Generate a SOC shift handoff report template with the following sections:
...
10. Freeform notes and observations`,
    useWhen: "Standardizing shift handoff communications in the SOC.",
    variables: ["DAY/SWING/NIGHT", "DATE", "NAMES"]
  },
  {
    id: 103,
    title: "SOC Daily Operations Report",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Generate a SOC daily operations report for [DATE]:
...
9. Week-over-week trend comparison`,
    useWhen: "Creating daily SOC status reports for management.",
    variables: ["DATE", "SIEM_PLATFORM", "TICKETING_SYSTEM", "TI_SOURCES"]
  },
  {
    id: 104,
    ...
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create an escalation decision matrix for SOC analysts at [ORGANIZATION]:
...
9. After-hours and weekend escalation modifications`,
    useWhen: "Clarifying when and how SOC analysts should escalate.",
    variables: ["ORGANIZATION", "CRITERIA", "SLA"]
  },
  {
    id: 105,
    title: "Weekly SOC Performance Review",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create a weekly SOC performance review report for [WEEK_DATE_RANGE]:
...
8. Recommendations for next week`,
    useWhen: "Weekly SOC operational reviews and continuous improvement.",
    variables: ["WEEK_DATE_RANGE", "SYSTEM", "PLATFORM"]
  },
  {
    id: 106,
    title: "SOC Analyst Onboarding Checklist",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create a SOC analyst onboarding checklist for a new [TIER_LEVEL] analyst at [ORGANIZATION]:
...
12. 30/60/90 day milestone definitions`,
    useWhen: "Onboarding new SOC team members.",
    variables: ["TIER_LEVEL", "ORGANIZATION", "LIST_SOC_TOOLS"]
  },
  {
    id: 107,
    title: "SOC Playbook: Insider Threat Alert",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create a SOC playbook for handling insider threat alerts from [DLP/UBA_TOOL]:
...
10. Hand-off to investigations team if warranted`,
    useWhen: "Handling sensitive insider threat alerts that require careful procedures.",
    variables: ["DLP/UBA_TOOL", "DATA_EXFILTRATION/POLICY_VIOLATION/BEHAVIORAL_ANOMALY"]
  },
  {
    id: 108,
    title: "Threat Briefing Generator",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Generate a threat briefing for [ORGANIZATION]'s security leadership:
...
8. Intelligence gaps and collection priorities`,
    useWhen: "Preparing regular threat briefings for security leadership.",
    variables: ["ORGANIZATION", "INDUSTRY", "REGIONS_OF_OPERATION", "ANY_RELEVANT_INCIDENTS", "LEVEL", "TECH_STACK"]
  },
  {
    id: 109,
    title: "SOC Tool Health Monitoring Checklist",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create a SOC tool health monitoring checklist for daily verification:
...
10. Escalation contacts for each tool if issues found`,
    useWhen: "Daily verification of SOC infrastructure health.",
    variables: []
  },
  {
    id: 110,
    title: "Tabletop Exercise Scenario Design",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Design a tabletop exercise scenario for the SOC team at [ORGANIZATION]:
...
10. Improvement action item template`,
    useWhen: "Running SOC readiness exercises and tabletop drills.",
    variables: ["ORGANIZATION", "RANSOMWARE/DATA_BREACH/APT/INSIDER_THREAT/SUPPLY_CHAIN", "ROLES_LIST"]
  },
  {
    id: 111,
    title: "SOC Metrics Definition Guide",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Define operational metrics for [ORGANIZATION]'s SOC:
...
4. For each metric: formula, data source, collection frequency, target value, and improvement actions if below target`,
    useWhen: "Establishing or maturing SOC performance measurement.",
    variables: ["ORGANIZATION", "INITIAL/MANAGED/DEFINED/MEASURED/OPTIMIZING"]
  },
  {
    id: 112,
    title: "SOC Communication Protocol",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Establish SOC communication protocols for [ORGANIZATION]:
...
10. Meeting cadence: daily standup, weekly review, monthly deep-dive`,
    useWhen: "Formalizing SOC communication standards.",
    variables: ["ORGANIZATION", "SLACK/TEAMS/RADIO/PHONE/TICKETING"]
  },
  {
    id: 113,
    title: "SOC Knowledge Base Article Template",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create a SOC knowledge base article template for documenting [TOPIC_TYPE: tool procedure / investigation technique / common alert]:
...
- [FREQUENT_INVESTIGATION_SCENARIO]`,
    useWhen: "Building SOC documentation and knowledge management.",
    variables: ["COMMON_ALERT_TYPE", "TOOL_NAME", "FREQUENT_INVESTIGATION_SCENARIO"]
  },
  {
    id: 114,
    title: "After-Hours Incident Response Protocol",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Design an after-hours incident response protocol for [ORGANIZATION]:
...
10. Quarterly on-call effectiveness review template`,
    useWhen: "Establishing or improving after-hours SOC coverage.",
    variables: ["ORGANIZATION", "TEAM_SIZE"]
  },
  {
    id: 115,
    title: "SOAR Playbook: Account Compromise",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Design a SOAR playbook for automated account compromise response:
...
10. Metrics: track playbook execution time and accuracy`,
    useWhen: "Automating account compromise response with SOAR.",
    variables: ["PLATFORM", "IDP", "SIEM_PLATFORM", "SPECIFIC_ALERT_CONDITION"]
  },
  {
    id: 116,
    title: "SOC Capacity Planning Model",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Build a SOC capacity planning model for [ORGANIZATION]:
...
10. Budget estimate for recommended model`,
    useWhen: "Planning SOC staffing and resource allocation.",
    variables: ["ORGANIZATION", "DAILY_AVERAGE", "MONTHLY_AVERAGE", "ANNUAL_PERCENTAGE"]
  },
  {
    id: 117,
    title: "Purple Team Detection Validation Report",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create a detection validation report from a purple team exercise:
...
9. 90-day follow-up validation plan`,
    useWhen: "Documenting detection validation exercise results.",
    variables: ["LIST_MITRE_TECHNIQUE_IDS", "RED_TEAM_TOOLS"]
  },
  {
    id: 118,
    title: "SOC Standard Operating Procedure Template",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Write a standard operating procedure (SOP) for [SOC_PROCESS]:
...
12. Approval signatures section`,
    useWhen: "Formalizing SOC processes into documented procedures.",
    variables: ["SOC_PROCESS", "ALERT_HANDLING/INCIDENT_ESCALATION/EVIDENCE_COLLECTION/TOOL_MAINTENANCE", "QUARTERLY/ANNUALLY"]
  },
  {
    id: 119,
    title: "Incident Post-Mortem Report Template",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create an incident post-mortem report for incident [INCIDENT_ID]:
...
10. Lessons learned and systemic improvements`,
    useWhen: "Documenting incidents for organizational learning.",
    variables: ["INCIDENT_ID", "BRIEF_DESCRIPTION", "LEVEL", "DETECTION_TO_RESOLUTION_TIME"]
  },
  {
    id: 120,
    title: "Threat Intelligence Platform (TIP) Operations Playbook",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Design a Threat Intelligence Platform operations playbook for [ORGANIZATION] using [TIP_PLATFORM: MISP/OpenCTI/ThreatConnect/Anomali/etc.]:
...
5. Quality assurance: periodic feed value assessment and pruning`,
    useWhen: "Standing up or improving threat intelligence platform operations.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 121,
    title: "Forensic Evidence Handling SOP",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create a forensic evidence handling standard operating procedure for the SOC at [ORGANIZATION]:
...
5. Legal considerations: admissibility requirements, privacy law compliance for [JURISDICTION]`,
    useWhen: "Establishing or auditing forensic evidence handling procedures.",
    variables: ["ORGANIZATION", "JURISDICTION"]
  },
  {
    id: 122,
    title: "SOC Technology Stack Evaluation",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Evaluate and recommend a SOC technology stack for [ORGANIZATION]:
...
10. Recommended stack with justification`,
    useWhen: "Planning SOC technology investments or migrations.",
    variables: ["ORGANIZATION", "EMPLOYEE_COUNT", "INDUSTRY", "LIST_EXISTING_TOOLS", "STARTUP/MID/ENTERPRISE", "PREFERENCE"]
  },
  {
    id: 123,
    title: "Chronicle SIEM Detection Engineering",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Write detection rules for Google Chronicle SIEM targeting these threat scenarios in [ORGANIZATION]:
...
- Tuning parameters to reduce false positives`,
    useWhen: "Building detection content in Google Chronicle SIEM.",
    variables: ["ORGANIZATION", "THRESHOLD", "TIMEFRAME"]
  },
  {
    id: 124,
    title: "QRadar Offense Investigation Workflow",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create a QRadar offense investigation workflow for the SOC at [ORGANIZATION]:
...
4. Performance:
   ...
   - Dashboard templates for offense volume and MTTR tracking`,
    useWhen: "Standardizing QRadar offense handling procedures.",
    variables: ["ORGANIZATION", "SOAR_PLATFORM"]
  },
  {
    id: 125,
    title: "DFIR Toolkit and Procedure Guide",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Build a Digital Forensics and Incident Response toolkit guide for [ORGANIZATION]:
...
5. Tool validation: annual testing and hash verification schedule`,
    useWhen: "Building or updating DFIR capabilities and toolkits.",
    variables: ["ORGANIZATION", "RANSOMWARE/BEC/INSIDER/APT", "WINDOWS/LINUX/MACOS"]
  },
  {
    id: 126,
    title: "SOC Automation Maturity Roadmap",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Design a SOC automation maturity roadmap for [ORGANIZATION]:
...
5. Risk assessment: automation failure modes and rollback procedures`,
    useWhen: "Planning progressive SOC automation improvements.",
    variables: ["ORGANIZATION", "HEADCOUNT", "PLATFORM", "PLATFORM_OR_NONE", "NONE/BASIC/INTERMEDIATE/ADVANCED"]
  },
  {
    id: 127,
    title: "Security Operations Metrics Dashboard",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Design a comprehensive SOC metrics and KPI dashboard for [ORGANIZATION]:
...
Include: recommended thresholds, alerting on metric degradation, and quarterly trend reporting format.`,
    useWhen: "Building or improving SOC performance measurement.",
    variables: ["ORGANIZATION", "TIMEFRAME"]
  },
  {
    id: 128,
    title: "Threat Hunting Campaign Framework",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Design a structured threat hunting campaign for [ORGANIZATION] targeting [THREAT_HYPOTHESIS]:
...
5. Campaign metrics: hours invested, findings count, detections created, coverage gaps identified`,
    useWhen: "Planning and executing structured threat hunting operations.",
    variables: ["ORGANIZATION", "THREAT_HYPOTHESIS", "APT_GROUP/THREAT_TYPE", "TECHNIQUE_IDS", "LOG_TYPES", "THREAT_ACTIVITY", "REASONING", "SIEM_PLATFORM", "LOOKBACK_PERIOD"]
  },
  {
    id: 129,
    title: "SOC Runbook for Cloud Security Events",
    category: "soc-ops",
    subcategory: "Shift Operations & Reporting",
    prompt: `Create SOC runbooks for handling cloud security events across [AWS/AZURE/GCP]:
...
For each: SLA targets, required access/permissions, communication templates.`,
    useWhen: "Standardizing SOC response to cloud-specific security events.",
    variables: ["AWS/AZURE/GCP"]
  },
  {
    id: 130,
    title: "AWS IAM Policy Security Review",
    category: "cloud-security",
    subcategory: "AWS Security",
    prompt: `Review the following AWS IAM policy for security issues:
...
9. Policy simulation: what would this policy allow an attacker to do?`,
    useWhen: "Reviewing IAM policies for least privilege compliance.",
    variables: ["PASTE_IAM_POLICY_JSON"]
  },
  {
    id: 131,
    title: "AWS Security Hub Findings Triage",
    category: "cloud-security",
    subcategory: "AWS Security",
    prompt: `Analyze and prioritize these AWS Security Hub findings:
...
8. Identify findings that can be auto-remediated vs. manual`,
    useWhen: "Working through Security Hub finding backlogs.",
    variables: ["PASTE_FINDINGS_JSON_OR_SUMMARY", "ENVIRONMENT_CONTEXT"]
  },
  {
    id: 132,
    title: "AWS CloudTrail Log Investigation",
    category: "cloud-security",
    subcategory: "AWS Security",
    prompt: `Analyze these AWS CloudTrail events for suspicious activity:
...
9. Recommended immediate actions based on findings`,
    useWhen: "Investigating potential AWS account compromise.",
    variables: ["PASTE_CLOUDTRAIL_EVENTS"]
  },
  {
    id: 133,
    title: "AWS S3 Bucket Security Audit",
    category: "cloud-security",
    subcategory: "AWS Security",
    prompt: `Audit S3 bucket security for our AWS account [ACCOUNT_ID]:
...
10. Remediation commands for each finding`,
    useWhen: "Auditing S3 security posture across an AWS account.",
    variables: ["ACCOUNT_ID", "BUCKET_LIST_OR_INSTRUCTIONS"]
  },
  {
    id: 134,
    title: "AWS GuardDuty Finding Response",
    category: "cloud-security",
    subcategory: "AWS Security",
    prompt: `Create response procedures for this AWS GuardDuty finding:
...
8. GuardDuty tuning: should this finding type be suppressed?`,
    useWhen: "Responding to GuardDuty alerts in AWS environments.",
    variables: ["FINDING_TYPE", "RESOURCE_ARN", "PASTE_FINDING_DETAILS"]
  },
  {
    id: 135,
    title: "AWS Network Security Assessment",
    category: "cloud-security",
    subcategory: "AWS Security",
    prompt: `Assess network security for AWS VPC [VPC_ID] in [REGION]:
...
9. Remediation priorities ranked by risk`,
    useWhen: "Auditing AWS network security architecture.",
    variables: ["VPC_ID", "REGION", "DESCRIBE_OR_PASTE_VPC_CONFIGURATION"]
  },
  {
    id: 136,
    ...
    prompt: `Assess Microsoft Entra ID (formerly Azure AD) security for tenant [TENANT_NAME]:
...
10. Prioritized remediation roadmap`,
    useWhen: "Auditing Microsoft Entra ID security configuration.",
    variables: ["TENANT_NAME", "PASTE_KEY_SETTINGS_OR_DESCRIBE"]
  },
  {
    id: 137,
    ...
    category: "cloud-security",
    subcategory: "Azure Security",
    prompt: `Review security configuration for Azure subscription [SUBSCRIPTION_ID]:
...
10. Specific az CLI commands to check each item`,
    useWhen: "Comprehensive Azure subscription security review.",
    variables: ["SUBSCRIPTION_ID", "LIST_KEY_RESOURCES"]
  },
  {
    id: 138,
    title: "Microsoft Sentinel Analytic Rules for Cloud Threats",
    category: "cloud-security",
    subcategory: "Azure Security",
    prompt: `Write Microsoft Sentinel KQL analytic rules for detecting cloud-specific threats:
...
For each: KQL query, entity mapping, severity, and investigation guidance.`,
    useWhen: "Building cloud threat detection in Microsoft Sentinel.",
    variables: []
  },
  {
    id: 139,
    title: "GCP Organization Security Assessment",
    category: "cloud-security",
    subcategory: "GCP Security",
    prompt: `Assess security posture for GCP organization [ORG_ID]:
...
10. Provide gcloud CLI commands for each check`,
    useWhen: "Auditing GCP organization security.",
    variables: ["ORG_ID", "PROJECT_LIST", "SERVICE_LIST"]
  },
  {
    id: 140,
    title: "GCP Workload Identity and Service Account Security",
    category: "cloud-security",
    subcategory: "GCP Security",
    prompt: `Review GCP service account security for project [PROJECT_ID]:
...
8. Exact gcloud commands to check and remediate each issue`,
    useWhen: "Auditing GCP service account security.",
    variables: ["PROJECT_ID", "LIST_OR_DESCRIBE"]
  },
  {
    id: 141,
    title: "Multi-Cloud Security Posture Comparison",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Compare security posture across our multi-cloud environment:
...
10. Single-pane-of-glass monitoring strategy`,
    useWhen: "Assessing security consistency across multi-cloud environments.",
    variables: ["LIST"]
  },
  {
    id: 142,
    ...
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Audit the security of Kubernetes cluster [CLUSTER_NAME] on [PLATFORM: EKS/AKS/GKE/self-managed]:
...
11. Prioritized remediation actions`,
    useWhen: "Comprehensive Kubernetes cluster security review.",
    variables: ["CLUSTER_NAME"]
  },
  {
    id: 143,
    title: "Container Image Security Pipeline",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Design a container image security pipeline for [CI_CD_PLATFORM]:
...
10. Monitoring deployed container vulnerabilities`,
    useWhen: "Building secure container image pipelines.",
    variables: ["CI_CD_PLATFORM", "DESCRIBE_BUILD_AND_DEPLOY_PROCESS"]
  },
  {
    id: 144,
    ...
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Review security of serverless functions in [AWS_LAMBDA/AZURE_FUNCTIONS/GOOGLE_CLOUD_FUNCTIONS]:
...
10. Remediation recommendations per function`,
    useWhen: "Auditing serverless function security.",
    variables: ["AWS_LAMBDA/AZURE_FUNCTIONS/GOOGLE_CLOUD_FUNCTIONS", "LIST_WITH_RUNTIMES_AND_TRIGGERS"]
  },
  {
    id: 145,
    title: "Cloud Infrastructure as Code Security Review",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Review this Infrastructure as Code template for security issues:
...
10. Recommend tfsec/checkov/cfn-nag rules to prevent these issues`,
    useWhen: "Security reviewing Infrastructure as Code before deployment.",
    variables: ["TERRAFORM/CLOUDFORMATION/BICEP/PULUMI", "PASTE_TEMPLATE_OR_KEY_SECTIONS"]
  },
  {
    id: 146,
    title: "Cloud Cost and Security Optimization",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Identify security improvements that also reduce cloud costs in [CLOUD_PROVIDER]:
...
8. Implementation priority considering both security and cost impact`,
    useWhen: "Aligning security improvement with cost optimization initiatives.",
    variables: ["CLOUD_PROVIDER", "DESCRIBE_KEY_RESOURCES_AND_ARCHITECTURE"]
  },
  {
    id: 147,
    title: "Cloud Incident Response Preparation",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Prepare cloud-specific incident response capabilities for [CLOUD_PROVIDER]:
...
8. Tabletop scenario specific to cloud compromise`,
    useWhen: "Building cloud-specific IR capabilities.",
    variables: ["CLOUD_PROVIDER", "DESCRIPTION", "LIST", "BEGINNER/INTERMEDIATE/ADVANCED"]
  },
  {
    id: 148,
    title: "Service Mesh Security Configuration",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Review security configuration of [ISTIO/LINKERD/CONSUL_CONNECT] service mesh in cluster [CLUSTER_NAME]:
...
9. Exact configuration commands to harden each area`,
    useWhen: "Hardening service mesh security in Kubernetes environments.",
    variables: ["ISTIO/LINKERD/CONSUL_CONNECT", "CLUSTER_NAME", "PASTE_KEY_CONFIGS_OR_DESCRIBE"]
  },
  {
    id: 149,
    title: "Cloud Database Security Audit",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Audit security for cloud database [DB_SERVICE] in [CLOUD_PROVIDER]:
...
10. Remediation commands for findings`,
    useWhen: "Auditing cloud-hosted database security.",
    variables: ["DB_SERVICE", "CLOUD_PROVIDER", "DATA_SENSITIVITY_LEVEL"]
  },
  {
    id: 150,
    title: "Cloud Security Architecture for New Workload",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Design cloud security architecture for deploying [WORKLOAD_DESCRIPTION] on [CLOUD_PROVIDER]:
...
10. Deployment checklist with security gates`,
    useWhen: "Designing secure cloud architectures for new deployments.",
    variables: ["WORKLOAD_DESCRIPTION", "CLOUD_PROVIDER", "LEVEL", "FRAMEWORKS", "INTERNAL/EXTERNAL/BOTH", "SLA_REQUIREMENT"]
  },
  {
    id: 151,
    title: "Cloud Privilege Escalation Detection",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Build detection rules for cloud privilege escalation in [AWS/AZURE/GCP]:
...
8. For each: detection query, severity, investigation steps, and false positive guidance`,
    useWhen: "Building cloud-specific privilege escalation detection.",
    variables: ["AWS/AZURE/GCP", "SIEM/CLOUD_NATIVE_TOOLS"]
  },
  {
    id: 152,
    title: "Cloud Compliance Benchmark Automation",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Automate compliance checks for [CIS_BENCHMARK/SOC2/HIPAA/PCI] in [CLOUD_PROVIDER]:
...
8. Drift detection: alerting when compliant resources become non-compliant`,
    useWhen: "Automating cloud compliance monitoring.",
    variables: ["CIS_BENCHMARK/SOC2/HIPAA/PCI", "CLOUD_PROVIDER"]
  },
  {
    id: 153,
    title: "Cloud Secrets Management Strategy",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Design a secrets management strategy for our cloud environment on [CLOUD_PROVIDERS]:
...
8. Secret sprawl detection (scanning for hardcoded secrets)`,
    useWhen: "Centralizing and securing secret management in cloud environments.",
    variables: ["CLOUD_PROVIDERS", "DESCRIPTION", "LIST"]
  },
  {
    id: 154,
    title: "Cloud WAF Configuration Review",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Review WAF configuration for [AWS_WAF/AZURE_WAF/CLOUDFLARE/CLOUD_ARMOR]:
...
10. Specific rule additions or modifications recommended`,
    useWhen: "Auditing and optimizing cloud WAF configurations.",
    variables: ["AWS_WAF/AZURE_WAF/CLOUDFLARE/CLOUD_ARMOR", "PASTE_CONFIG_OR_DESCRIBE", "LIST_OF_ENDPOINTS"]
  },
  {
    id: 155,
    title: "Cloud Forensic Readiness Assessment",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Assess forensic readiness of our [CLOUD_PROVIDER] environment:
...
9. Forensic readiness improvement roadmap`,
    useWhen: "Ensuring cloud environments support forensic investigations.",
    variables: ["CLOUD_PROVIDER", "DESCRIPTION", "WHAT_IS_ENABLED"]
  },
  {
    id: 156,
    title: "Cloud Network Microsegmentation Design",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Design a network microsegmentation strategy for [CLOUD_WORKLOAD]:
...
8. Operational procedures for change management`,
    useWhen: "Implementing microsegmentation in cloud workloads.",
    variables: ["CLOUD_WORKLOAD", "DESCRIBE_WORKLOAD_COMPONENTS_AND_COMMUNICATION_FLOWS"]
  },
  {
    id: 157,
    title: "Cloud Identity Federation Security Review",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Review identity federation security between [IDP: Okta/PingFederate/ADFS/etc.] and [CLOUD_PROVIDER]:
...
9. Recommendations for hardening`,
    useWhen: "Auditing cloud identity federation configurations.",
    variables: ["CLOUD_PROVIDER", "DESCRIBE_SETUP"]
  },
  {
    id: 158,
    title: "Cloud CIEM Assessment",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Conduct a Cloud Infrastructure Entitlement Management assessment for [ORGANIZATION] across [AWS/AZURE/GCP/MULTI-CLOUD]:
...
5. CIEM tool evaluation: Tenable Cloud Security (formerly Ermetic) vs Microsoft Entra Permissions Management (formerly CloudKnox) vs CrowdStrike Falcon Cloud Security vs native tools (AWS IAM Access Analyzer, Azure Entra Permissions Management, GCP IAM Recommender)`,
    useWhen: "Assessing and reducing cloud identity and entitlement risks.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 159,
    title: "Cloud Security Posture Management (CSPM) Evaluation",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Evaluate CSPM coverage and effectiveness for [ORGANIZATION] across [CLOUD_PROVIDERS]:
...
5. Tool comparison: Wiz (acquired Orca Security, March 2025) vs Prisma Cloud vs Aqua vs native (Security Hub, Defender for Cloud, SCC)
   - Feature matrix, cost model, deployment complexity, API coverage`,
    useWhen: "Evaluating or improving cloud security posture management.",
    variables: ["ORGANIZATION", "CLOUD_PROVIDERS", "TOOL_OR_NONE"]
  },
  {
    id: 160,
    title: "GCP Security Command Center Investigation",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Create investigation procedures using GCP Security Command Center for [ORGANIZATION]:
...
5. Compliance monitoring:
   ...
   - Cross-project security posture comparison`,
    useWhen: "Operating GCP Security Command Center for security monitoring and response.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 161,
    title: "Cloud Data Security Architecture",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Design a cloud data security architecture for [ORGANIZATION] handling [DATA_TYPES] in [CLOUD_PROVIDERS]:
...
5. Backup and recovery:
   ...
   - Recovery testing schedule and validation`,
    useWhen: "Designing comprehensive data security for cloud environments.",
    variables: ["ORGANIZATION", "DATA_TYPES", "CLOUD_PROVIDERS"]
  },
  {
    id: 162,
    title: "Cloud Workload Protection Platform Assessment",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Assess cloud workload protection needs for [ORGANIZATION]:
...
5. Tool evaluation: CrowdStrike Falcon Cloud vs Prisma Cloud Compute vs Aqua vs Sysdig Secure (note: Lacework was acquired by Fortinet in 2024)`,
    useWhen: "Evaluating cloud workload protection requirements and solutions.",
    variables: ["ORGANIZATION", "COUNT", "PROVIDERS", "LIST"]
  },
  {
    id: 163,
    title: "Cloud API Gateway Security Configuration",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Review and harden API gateway security configuration for [API_GATEWAY_SERVICE: AWS API Gateway/Azure API Management/GCP API Gateway/Kong]:
...
6. Recommendations ranked by risk and implementation effort`,
    useWhen: "Securing cloud API gateway configurations.",
    variables: ["API_LIST_WITH_ENDPOINTS"]
  },
  {
    id: 164,
    title: "Cloud FinOps Security Alignment",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Align cloud FinOps practices with security requirements for [ORGANIZATION]:
...
5. Budget allocation: recommended security spend as percentage of total cloud spend`,
    useWhen: "Aligning cloud financial operations with security objectives.",
    variables: ["ORGANIZATION", "MONTHLY_SPEND", "AWS/AZURE/GCP"]
  },
  {
    id: 165,
    title: "Cloud Disaster Recovery Security Review",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Review security controls for cloud disaster recovery at [ORGANIZATION]:
...
5. Recovery validation:
   ...
   - Incident response procedures for DR scenarios`,
    useWhen: "Ensuring disaster recovery plans maintain security standards.",
    variables: ["ORGANIZATION", "REGION", "TARGET", "PILOT_LIGHT/WARM_STANDBY/MULTI_SITE/BACKUP_RESTORE"]
  },
  {
    id: 166,
    title: "Cloud-Native Application Protection Platform (CNAPP) Strategy",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Develop a CNAPP strategy for [ORGANIZATION]:
...
5. Vendor consolidation: single CNAPP vs best-of-breed evaluation
   - Wiz (acquired Orca Security, March 2025) vs Prisma Cloud vs native tools matrix`,
    useWhen: "Building a comprehensive cloud-native application security strategy.",
    variables: ["ORGANIZATION", "COUNT"]
  },
  {
    id: 167,
    title: "Cloud Zero Trust Network Architecture",
    category: "cloud-security",
    subcategory: "Multi-Cloud & Container Security",
    prompt: `Design a Zero Trust network architecture for cloud workloads at [ORGANIZATION]:
...
5. Migration roadmap: phased implementation from highest-risk segments first`,
    useWhen: "Implementing Zero Trust networking principles in cloud environments.",
    variables: ["ORGANIZATION", "LIST", "HUB_SPOKE/MESH/FLAT", "PERCENTAGE"]
  },
  {
    id: 168,
    title: "Domain Intelligence Report",
    category: "osint",
    subcategory: "Domain & IP Intelligence",
    prompt: `Compile a comprehensive OSINT intelligence report for [TARGET_DOMAIN]:
...
10. Summary with risk indicators and investigation leads`,
    useWhen: "Starting domain-level intelligence gathering.",
    variables: ["TARGET_DOMAIN"]
  },
  {
    id: 169,
    title: "IP Address Intelligence Analysis",
    category: "osint",
    subcategory: "Domain & IP Intelligence",
    prompt: `Analyze IP address [TARGET_IP] for threat intelligence:
...
10. Assessment: likely purpose and risk level of this IP`,
    useWhen: "Investigating suspicious IP addresses.",
    variables: ["TARGET_IP"]
  },
  {
    id: 170,
    title: "Infrastructure Mapping from OSINT",
    category: "osint",
    subcategory: "Domain & IP Intelligence",
    prompt: `Map the external infrastructure of [TARGET_ORGANIZATION] using only passive OSINT:
...
10. Attack surface summary with risk ratings`,
    useWhen: "Mapping an organization\'s external attack surface.",
    variables: ["TARGET_ORGANIZATION", "DOMAIN", "IF_ANY"]
  },
  {
    id: 171,
    ...
    category: "osint",
    subcategory: "Domain & IP Intelligence",
    prompt: `Analyze certificate transparency logs for [TARGET_DOMAIN]:
...
10. Intelligence summary and new investigation leads`,
    useWhen: "Mining certificate transparency for intelligence.",
    variables: ["TARGET_DOMAIN"]
  },
  {
    id: 172,
    title: "DNS Passive Intelligence Gathering",
    category: "osint",
    subcategory: "Domain & IP Intelligence",
    prompt: `Conduct passive DNS intelligence gathering for [TARGET_DOMAIN]:
...
10. Build infrastructure change timeline`,
    useWhen: "Passive DNS research for investigation or reconnaissance.",
    variables: ["TARGET_DOMAIN"]
  },
  {
    id: 173,
    title: "Shodan/Censys Intelligence Query Design",
    category: "osint",
    subcategory: "Domain & IP Intelligence",
    prompt: `Design targeted Shodan and Censys queries for intelligence on [TARGET_ORGANIZATION]:
...
10. Risk summary of exposed attack surface`,
    useWhen: "Conducting internet-wide scan database research.",
    variables: ["TARGET_ORGANIZATION", "DOMAINS", "IF_KNOWN"]
  },
  {
    id: 174,
    title: "Corporate Digital Footprint Assessment",
    category: "osint",
    subcategory: "Digital Footprint Analysis",
    prompt: `Assess the digital footprint of [TARGET_ORGANIZATION]:
...
10. Risk summary with specific actionable findings`,
    useWhen: "Comprehensive digital footprint assessment of an organization.",
    variables: ["TARGET_ORGANIZATION"]
  },
  {
    id: 175,
    title: "Employee OSINT for Security Assessment",
    category: "osint",
    subcategory: "Digital Footprint Analysis",
    prompt: `Conduct an OSINT assessment of employees at [TARGET_ORGANIZATION] for an authorized security engagement:
...
Note: This is for authorized assessment only. Adhere to legal boundaries and engagement scope.`,
    useWhen: "Authorized employee OSINT for social engineering planning.",
    variables: ["TARGET_ORGANIZATION"]
  },
  {
    id: 176,
    title: "Data Leak Detection Methodology",
    category: "osint",
    subcategory: "Digital Footprint Analysis",
    prompt: `Design a methodology to detect data leaks for [ORGANIZATION]:
...
10. Metrics: monitoring coverage and response time`,
    useWhen: "Building data leak detection capabilities.",
    variables: ["ORGANIZATION", "TYPES", "REPO_NAMES", "SYSTEMS", "DESCRIPTION"]
  },
  {
    id: 177,
    ...
    category: "osint",
    subcategory: "Digital Footprint Analysis",
    prompt: `Design a brand impersonation detection program for [ORGANIZATION]:
...
10. Legal escalation framework`,
    useWhen: "Protecting brand integrity from impersonation attacks.",
    variables: ["ORGANIZATION", "DOMAIN", "PRODUCTS", "KEY_EXECUTIVES", "DESCRIPTION"]
  },
  {
    id: 178,
    title: "Google Dorking Playbook",
    category: "osint",
    subcategory: "Digital Footprint Analysis",
    prompt: `Create a comprehensive Google dorking playbook for investigating [TARGET_DOMAIN] as part of an authorized security assessment.
...
For each: explanation of what it finds and risk level.`,
    useWhen: "Conducting authorized Google dorking reconnaissance for defensive assessments.",
    variables: ["TARGET_DOMAIN", "DOMAIN", "COMPANY_NAME"]
  },
  {
    id: 179,
    title: "Supply Chain OSINT Investigation",
    category: "osint",
    subcategory: "Digital Footprint Analysis",
    prompt: `Investigate the supply chain of [TARGET_ORGANIZATION] through OSINT:
...
10. Recommendations for supply chain security monitoring`,
    useWhen: "Assessing third-party risk through OSINT.",
    variables: ["TARGET_ORGANIZATION", "INDUSTRY", "DESCRIPTION"]
  },
  {
    id: 180,
    title: "Threat Actor Profile Development",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Develop a threat actor profile for [THREAT_ACTOR_NAME_OR_CAMPAIGN]:
...
11. Intelligence gaps and assessment confidence levels`,
    useWhen: "Building threat profiles for intelligence-informed defense.",
    variables: ["THREAT_ACTOR_NAME_OR_CAMPAIGN"]
  },
  {
    id: 181,
    ...
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Analyze the cyber campaign [CAMPAIGN_NAME/DESCRIPTION]:
...
11. IOCs formatted for ingestion (STIX/OpenIOC format)`,
    useWhen: "Analyzing active threat campaigns.",
    variables: ["CAMPAIGN_NAME/DESCRIPTION", "PASTE_AVAILABLE_IOCS_AND_TTPS"]
  },
  {
    id: 182,
    title: "Dark Web Intelligence Collection Plan",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Design a dark web intelligence collection plan for [ORGANIZATION]:
...
10. Metrics: collection volume, actionable finds rate`,
    useWhen: "Establishing dark web monitoring capabilities.",
    variables: ["ORGANIZATION", "DOMAINS", "ORGANIZATION_OR_INDUSTRY"]
  },
  {
    id: 183,
    title: "Threat Intelligence Requirements Document",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Create a threat intelligence requirements (TIR) document for [ORGANIZATION]:
...
8. Annual review process for updating requirements`,
    useWhen: "Formalizing threat intelligence program requirements.",
    variables: ["ORGANIZATION", "INDUSTRY", "OPERATIONS_LOCATIONS", "LIST"]
  },
  {
    id: 184,
    title: "OSINT Investigation Report Template",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Create an OSINT investigation report for [INVESTIGATION_SUBJECT]:
...
11. Appendix: raw data, screenshots, tool output`,
    useWhen: "Documenting OSINT investigation findings formally.",
    variables: ["INVESTIGATION_SUBJECT", "INCIDENT_SUPPORT/THREAT_ASSESSMENT/DUE_DILIGENCE", "DEFINE_BOUNDARIES", "DATE_RANGE"]
  },
  {
    id: 185,
    title: "Geopolitical Cyber Threat Assessment",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Assess geopolitical cyber threats relevant to [ORGANIZATION]:
...
9. Intelligence gaps requiring monitoring`,
    useWhen: "Assessing geopolitical cyber risk for strategic planning.",
    variables: ["ORGANIZATION", "INDUSTRY", "COUNTRY", "COUNTRIES_OF_OPERATION", "SUPPLY_CHAIN_COUNTRIES"]
  },
  {
    id: 186,
    title: "Malware OSINT Research Guide",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Guide me through OSINT research on malware sample [HASH/NAME]:
...
11. MITRE ATT&CK mapping of observed behaviors`,
    useWhen: "Researching malware samples using open source intelligence.",
    variables: ["HASH/NAME"]
  },
  {
    id: 187,
    title: "Social Media Intelligence (SOCMINT) Collection",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Design a social media intelligence collection plan for [INVESTIGATION_PURPOSE]:
...
10. Evidence preservation: archiving and chain of custody`,
    useWhen: "Conducting social media intelligence operations.",
    variables: ["INVESTIGATION_PURPOSE", "INDIVIDUAL/ORGANIZATION/TOPIC"]
  },
  {
    id: 188,
    title: "Attack Surface Discovery Automation",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Design an automated external attack surface discovery workflow for [ORGANIZATION]:
...
10. Scheduling: how often to run and how to track changes`,
    useWhen: "Building continuous external attack surface monitoring.",
    variables: ["ORGANIZATION", "LIST", "NAME"]
  },
  {
    id: 189,
    ...
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Analyze a potential disinformation campaign targeting [ORGANIZATION/TOPIC]:
...
10. Monitoring plan for ongoing tracking`,
    useWhen: "Investigating organized disinformation targeting your organization.",
    variables: ["ORGANIZATION/TOPIC", "DESCRIBE_OBSERVED_SOCIAL_MEDIA_ACTIVITY_OR_CONTENT"]
  },
  {
    id: 190,
    title: "Cryptocurrency OSINT Investigation",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Investigate cryptocurrency activity for [WALLET_ADDRESS/TRANSACTION_ID]:
...
10. Tools: Block explorers (Blockchain.com, Blockchair), Etherscan, Arkham Intelligence, Crystal Blockchain, Chainalysis (if available)`,
    useWhen: "Investigating cryptocurrency transactions in cyber investigations.",
    variables: ["WALLET_ADDRESS/TRANSACTION_ID", "BITCOIN/ETHEREUM/OTHER"]
  },
  {
    id: 191,
    title: "Physical Infrastructure OSINT",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct OSINT on the physical infrastructure of [TARGET_ORGANIZATION] as part of an authorized security engagement.
...
10. Risk assessment of physical infrastructure exposure`,
    useWhen: "Physical infrastructure intelligence for authorized comprehensive assessments.",
    variables: ["TARGET_ORGANIZATION"]
  },
  {
    id: 192,
    title: "Competitive Intelligence Security Assessment",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Assess the cybersecurity posture of [COMPETITOR/TARGET] using only public information:
...
10. Comparative analysis if assessing multiple organizations`,
    useWhen: "Due diligence, competitive analysis, or M&A security assessment.",
    variables: ["COMPETITOR/TARGET"]
  },
  {
    id: 193,
    title: "OSINT Workflow Automation Design",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Design an automated OSINT workflow for continuous monitoring of [MONITORING_TARGETS]:
...
10. Cost estimate for the recommended solution`,
    useWhen: "Building automated OSINT monitoring pipelines.",
    variables: ["MONITORING_TARGETS", "DOMAINS/BRANDS/KEYWORDS/EXECUTIVES", "WHAT_SHOULD_TRIGGER_NOTIFICATION", "REAL_TIME/DAILY/WEEKLY"]
  },
  {
    id: 194,
    title: "Email Infrastructure Investigation",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Investigate the email infrastructure of [TARGET_DOMAIN]:
...
10. Email spoofing viability assessment based on findings`,
    useWhen: "Investigating email security posture through OSINT.",
    variables: ["TARGET_DOMAIN"]
  },
  {
    id: 195,
    ...
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct wireless network OSINT for [TARGET_LOCATION]:
...
10. Risk assessment from wireless exposure`,
    useWhen: "Pre-engagement wireless reconnaissance using passive sources.",
    variables: ["TARGET_LOCATION"]
  },
  {
    id: 196,
    title: "AI-Generated Deepfake Detection in OSINT",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Develop a deepfake detection methodology for OSINT investigations at [ORGANIZATION]:
...
6. Training: analyst capability building for deepfake recognition`,
    useWhen: "Detecting AI-generated fake content during OSINT investigations.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 197,
    title: "IoT and Device OSINT Reconnaissance",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct IoT and connected device OSINT for [TARGET_ORGANIZATION]:
...
6. Reporting: prioritized findings with remediation recommendations`,
    useWhen: "Mapping IoT device exposure for authorized security assessments.",
    variables: ["TARGET_ORGANIZATION", "DEFINE_SCOPE"]
  },
  {
    id: 198,
    title: "Breach Credential Analysis Methodology",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct breach credential analysis for [TARGET_ORGANIZATION]:
...
5. Ongoing monitoring setup and alerting configuration`,
    useWhen: "Assessing organizational credential exposure from data breaches.",
    variables: ["TARGET_ORGANIZATION", "CONFIRM_LEGAL_AUTHORIZATION", "DOMAIN_LIST"]
  },
  {
    id: 199,
    title: "OSINT for Merger and Acquisition Due Diligence",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct cybersecurity OSINT due diligence for M&A target [TARGET_COMPANY]:
...
6. Integration risk: potential security gaps during integration period`,
    useWhen: "Performing cybersecurity due diligence on acquisition targets.",
    variables: ["TARGET_COMPANY"]
  },
  {
    id: 200,
    title: "Social Engineering Pretext OSINT",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct authorized OSINT to assess social engineering exposure for [TARGET_ORGANIZATION]:
...
5. Recommendations for reducing social engineering surface
   Note: Adhere to all legal boundaries including GDPR, CCPA, and local privacy laws.`,
    useWhen: "Assessing social engineering exposure through authorized OSINT.",
    variables: ["TARGET_ORGANIZATION", "CONFIRM_AUTHORIZED_ASSESSMENT"]
  },
  {
    id: 201,
    title: "Open Source Software Supply Chain OSINT",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct supply chain OSINT for open source dependencies used by [TARGET_ORGANIZATION]:
...
5. Monitoring: continuous dependency intelligence collection setup`,
    useWhen: "Assessing open source supply chain risks through OSINT.",
    variables: ["TARGET_ORGANIZATION", "LIST_KNOWN_TECHNOLOGIES"]
  },
  {
    id: 202,
    title: "Cloud Infrastructure OSINT",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct cloud infrastructure OSINT for [TARGET_ORGANIZATION]:
...
5. Risk assessment and prioritized findings report`,
    useWhen: "Mapping cloud infrastructure exposure through OSINT techniques.",
    variables: ["TARGET_ORGANIZATION", "DOMAIN_LIST"]
  },
  {
    id: 203,
    title: "Geopolitical Threat Landscape Monitoring",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Design a geopolitical threat landscape monitoring program for [ORGANIZATION] operating in [REGIONS]:
...
5. Response triggers: threshold criteria for activating heightened security posture`,
    useWhen: "Building geopolitical cyber threat monitoring capabilities.",
    variables: ["ORGANIZATION", "REGIONS", "INDUSTRY", "INDUSTRY/REGION"]
  },
  {
    id: 204,
    title: "OSINT Automation Pipeline Design",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Design an automated OSINT collection and analysis pipeline for [ORGANIZATION]:
...
6. Maintenance: pipeline health monitoring, source freshness checks`,
    useWhen: "Building automated OSINT collection and processing capabilities.",
    variables: ["ORGANIZATION", "LIST_PRIORITY_INTELLIGENCE_REQUIREMENTS"]
  },
  {
    id: 205,
    title: "Physical Security OSINT Assessment",
    category: "osint",
    subcategory: "Threat Actor Profiling",
    prompt: `Conduct physical security OSINT assessment for [TARGET_LOCATION]:
...
6. Risk assessment: physical intrusion path analysis with difficulty ratings`,
    useWhen: "Assessing physical security posture through authorized OSINT.",
    variables: ["TARGET_LOCATION", "CONFIRM_AUTHORIZED_ASSESSMENT"]
  },
  {
    id: 206,
    title: "Information Security Policy Framework",
    category: "grc",
    subcategory: "Policy Development",
    prompt: `Draft a comprehensive information security policy framework for [ORGANIZATION]:
...
11. Implementation roadmap and training requirements`,
    useWhen: "Building or restructuring the security policy framework.",
    variables: ["ORGANIZATION", "EMPLOYEE_COUNT", "INDUSTRY", "ISO_27001/NIST/SOC2/HIPAA/PCI/GDPR", "INITIAL/DEVELOPING/DEFINED/MANAGED/OPTIMIZING"]
  },
  {
    id: 207,
    title: "ISO 27001 Statement of Applicability Generator",
    category: "grc",
    subcategory: "Policy Development",
    prompt: `Generate an ISO 27001:2022 Statement of Applicability (SoA) for [ORGANIZATION]:
...
8. Priority for remediation (if gap exists)`,
    useWhen: "Creating or updating the ISO 27001 Statement of Applicability.",
    variables: ["ORGANIZATION", "ISMS_SCOPE", "JUSTIFIED_EXCLUSIONS"]
  },
  {
    id: 208,
    ...
    category: "grc",
    subcategory: "Policy Development",
    prompt: `Conduct a NIST CSF 2.0 assessment for [ORGANIZATION]:
...
9. Metrics to track progress per function`,
    useWhen: "Conducting NIST CSF assessments or maturity evaluations.",
    variables: ["ORGANIZATION", "DESCRIBE_CURRENT_SECURITY_PROGRAM", "RISK_APPETITE"]
  },
  {
    id: 209,
    ...
    category: "grc",
    subcategory: "Policy Development",
    prompt: `Draft a data classification policy for [ORGANIZATION]:
...
10. Training requirements`,
    useWhen: "Creating or updating data classification standards.",
    variables: ["ORGANIZATION", "LIST_KEY_DATA_TYPES", "EXISTS/NONE"]
  },
  {
    id: 210,
    ...
    category: "grc",
    subcategory: "Policy Development",
    prompt: `Create a Privacy Impact Assessment (PIA) template for [PROJECT/SYSTEM]:
...
12. Recommendations and approval sign-off`,
    useWhen: "Conducting privacy assessments for new projects or systems.",
    variables: ["PROJECT/SYSTEM", "WHAT_IT_DOES", "DATA_TYPES", "CONSENT/LEGITIMATE_INTEREST/CONTRACT/LEGAL_OBLIGATION"]
  },
  {
    id: 211,
    ...
    category: "grc",
    subcategory: "Policy Development",
    prompt: `Create a GDPR compliance checklist for [ORGANIZATION]:
...
11. For each: status, responsible party, evidence required, deadline`,
    useWhen: "Assessing and improving GDPR compliance posture.",
    variables: ["ORGANIZATION", "DESCRIPTION", "OVERVIEW", "ASSESSMENT"]
  },
  {
    id: 212,
    ...
    category: "grc",
    subcategory: "Policy Development",
    prompt: `Draft a third-party risk management policy for [ORGANIZATION]:
...
12. Annual review and reassessment process`,
    useWhen: "Establishing or maturing third-party risk management.",
    variables: ["ORGANIZATION", "ESTIMATE", "LIST", "REGULATIONS"]
  },
  {
    id: 213,
    title: "Risk Assessment Workshop Facilitation Guide",
    category: "grc",
    subcategory: "Risk Assessment",
    prompt: `Create a facilitation guide for a cybersecurity risk assessment workshop at [ORGANIZATION]:
...
12. Post-workshop deliverable outline`,
    useWhen: "Facilitating organizational risk assessment workshops.",
    variables: ["ORGANIZATION", "ASSESSMENT_SCOPE", "HALF_DAY/FULL_DAY"]
  },
  {
    id: 214,
    title: "Quantitative Risk Analysis (FAIR)",
    category: "grc",
    subcategory: "Risk Assessment",
    prompt: `Conduct a FAIR (Factor Analysis of Information Risk) assessment for:
...
8. Executive summary with dollar-denominated risk statement`,
    useWhen: "Quantifying cyber risk in financial terms for business decision-making.",
    variables: ["DESCRIBE_SCENARIO", "ASSET", "THREAT_ACTOR_TYPE"]
  },
  {
    id: 215,
    title: "Business Impact Analysis (BIA) for Cybersecurity",
    category: "grc",
    subcategory: "Risk Assessment",
    prompt: `Conduct a Business Impact Analysis for [ORGANIZATION]'s critical processes:
...
12. Gap analysis: current recovery capability vs. required`,
    useWhen: "Conducting BIA for business continuity and disaster recovery planning.",
    variables: ["ORGANIZATION", "LIST_CRITICAL_BUSINESS_PROCESSES"]
  },
  {
    id: 216,
    title: "Cyber Insurance Application Support",
    category: "grc",
    subcategory: "Risk Assessment",
    prompt: `Prepare supporting documentation for a cyber insurance application for [ORGANIZATION]:
...
For each area: current state summary, supporting evidence, and improvement plans.`,
    useWhen: "Preparing for cyber insurance applications or renewals.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 217,
    ...
    category: "grc",
    subcategory: "Risk Assessment",
    prompt: `Create a cybersecurity risk register for [ORGANIZATION]:
...
Populate with 10 common cybersecurity risks for [INDUSTRY] as examples.`,
    useWhen: "Establishing or refreshing the cybersecurity risk register.",
    variables: ["ORGANIZATION", "INDUSTRY"]
  },
  {
    id: 218,
    ...
    category: "grc",
    subcategory: "Risk Assessment",
    prompt: `Create a supply chain risk assessment framework for [ORGANIZATION]:
...
10. Annual reassessment process and cadence`,
    useWhen: "Building supply chain risk management capabilities.",
    variables: ["ORGANIZATION", "COUNT", "KEY_DEPENDENCIES", "INDUSTRY"]
  },
  {
    id: 219,
    title: "SOC 2 Type II Audit Preparation Checklist",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Create a SOC 2 Type II audit preparation checklist for [ORGANIZATION]:
...
4. Logistics: auditor access, interview scheduling, document sharing platform`,
    useWhen: "Preparing for SOC 2 audit engagements.",
    variables: ["ORGANIZATION", "SECURITY/AVAILABILITY/PROCESSING_INTEGRITY/CONFIDENTIALITY/PRIVACY", "DATE_RANGE", "FIRM_NAME"]
  },
  {
    id: 220,
    title: "PCI DSS v4.0 Gap Assessment",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Conduct a PCI DSS v4.0 gap assessment for [ORGANIZATION]:
...
10. Overall compliance percentage and target date`,
    useWhen: "Assessing PCI DSS v4.0 compliance posture.",
    variables: ["ORGANIZATION", "SAQ_TYPE", "ROC", "COMPLIANT/IN_PROGRESS/NEW"]
  },
  {
    id: 221,
    ...
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Review HIPAA Security Rule compliance for [ORGANIZATION]:
...
For each: required vs. addressable, current status, gap, remediation action, evidence needed.`,
    useWhen: "Assessing HIPAA Security Rule compliance.",
    variables: ["ORGANIZATION", "COVERED_ENTITY/BUSINESS_ASSOCIATE", "LIST", "OVERVIEW"]
  },
  {
    id: 222,
    title: "Security Compliance Evidence Collection Guide",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Create an evidence collection guide for [COMPLIANCE_FRAMEWORK] audit at [ORGANIZATION]:
...
11. Evidence freshness requirements (how recent is acceptable)`,
    useWhen: "Organizing evidence collection for compliance audits.",
    variables: ["COMPLIANCE_FRAMEWORK", "ORGANIZATION", "SCOPE", "DATE_RANGE"]
  },
  {
    id: 223,
    ...
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Assess the impact of [NEW_REGULATION_OR_AMENDMENT] on [ORGANIZATION]:
...
12. Recommended action plan with milestones`,
    useWhen: "Assessing impact of new or changed regulations.",
    variables: ["NEW_REGULATION_OR_AMENDMENT", "ORGANIZATION", "REGULATION_NAME", "DATE", "SUMMARY"]
  },
  {
    id: 224,
    title: "Internal Audit Cybersecurity Scope",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Define the scope for an internal cybersecurity audit at [ORGANIZATION]:
...
11. Conflict of interest declarations`,
    useWhen: "Planning internal cybersecurity audits.",
    variables: ["ORGANIZATION", "LIST_IF_AVAILABLE"]
  },
  {
    id: 225,
    ...
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Create a vendor security assessment questionnaire for [VENDOR_TIER: Critical/High/Standard]:
...
12. Scoring rubric per section with pass/fail criteria`,
    useWhen: "Evaluating vendor security posture.",
    variables: ["DATA_TYPES", "YES_NO_DESCRIPTION"]
  },
  {
    id: 226,
    title: "Security Exception Request Process",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Design a security exception/risk acceptance process for [ORGANIZATION]:
...
9. Escalation for overdue reviews or expired exceptions`,
    useWhen: "Establishing a formal process for security policy exceptions.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 227,
    ...
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Design a compliance dashboard for [ORGANIZATION] tracking [FRAMEWORKS]:
...
12. Refresh frequency and automation approach`,
    useWhen: "Building compliance monitoring dashboards.",
    variables: ["ORGANIZATION", "FRAMEWORKS"]
  },
  {
    id: 228,
    ...
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Generate a security awareness training compliance report for [ORGANIZATION]:
...
11. Executive summary for CISO`,
    useWhen: "Reporting on security training compliance.",
    variables: ["ORGANIZATION", "TRAINING_PLATFORM", "LIST", "DATE", "MONTHLY/QUARTERLY", "TARGET"]
  },
  {
    id: 229,
    title: "Board-Level Cybersecurity Report",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Create a board-level cybersecurity report for [ORGANIZATION]'s [QUARTER/YEAR]:
...
11. Recommended actions requiring board input/approval`,
    useWhen: "Preparing cybersecurity reports for board of directors.",
    variables: ["ORGANIZATION", "QUARTER/YEAR", "AMOUNT", "LIST", "IF_ANY", "AVAILABLE_DATA"]
  },
  {
    id: 230,
    title: "Security Policy Review and Update Process",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Design a security policy review and update process for [ORGANIZATION]:
...
10. Metrics: percentage of policies current, average age, overdue count`,
    useWhen: "Establishing systematic policy review and maintenance.",
    variables: ["ORGANIZATION", "COUNT", "LAST_REVIEW_DATES"]
  },
  {
    id: 231,
    title: "Incident Notification Regulatory Mapping",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Map incident notification requirements across applicable regulations for [ORGANIZATION]:
...
10. Create a single incident decision tree that maps any incident to all applicable notification requirements`,
    useWhen: "Mapping incident notification obligations across regulations.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 232,
    title: "Security Budget Allocation Framework",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Create a security budget allocation framework for [ORGANIZATION]:
...
11. Value demonstration metrics for each investment`,
    useWhen: "Planning and justifying security budget allocation.",
    variables: ["ORGANIZATION", "AMOUNT", "BREAKDOWN_IF_KNOWN", "LIST"]
  },
  {
    id: 233,
    title: "Security Maturity Assessment Report",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Conduct a cybersecurity maturity assessment for [ORGANIZATION] using [CMM/NIST/C2M2/custom]:
...
- Overall maturity score and peer comparison`,
    useWhen: "Baseline or periodic cybersecurity maturity assessments.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 234,
    title: "NIS2 Directive Compliance Assessment",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Conduct a NIS2 Directive compliance assessment for [ORGANIZATION] operating in [EU_MEMBER_STATES]:
...
5. Gap analysis with remediation roadmap and priority ranking`,
    useWhen: "Assessing compliance with the EU NIS2 Directive.",
    variables: ["ORGANIZATION", "EU_MEMBER_STATES", "ENERGY/TRANSPORT/BANKING/HEALTH/DIGITAL_INFRASTRUCTURE/ICT_SERVICE/PUBLIC_ADMIN/OTHER", "ESSENTIAL/IMPORTANT", "EMPLOYEES", "REVENUE"]
  },
  {
    id: 235,
    title: "DORA Compliance Assessment for Financial Entities",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Conduct a DORA (Digital Operational Resilience Act) compliance assessment for [FINANCIAL_ENTITY]:
...
6. Remediation roadmap with regulatory deadlines`,
    useWhen: "Assessing compliance with EU DORA regulation for financial sector entities.",
    variables: ["FINANCIAL_ENTITY", "CREDIT_INSTITUTION/INVESTMENT_FIRM/INSURANCE/PAYMENT_PROVIDER/CRYPTO_ASSET_PROVIDER/OTHER"]
  },
  {
    id: 236,
    title: "SEC Cyber Incident Disclosure Rules Compliance",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Assess compliance with SEC cybersecurity disclosure rules for [PUBLIC_COMPANY]:
...
5. Template: draft 8-K language for common incident types`,
    useWhen: "Ensuring compliance with SEC cybersecurity incident disclosure requirements.",
    variables: ["PUBLIC_COMPANY"]
  },
  {
    id: 237,
    ...
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Assess post-quantum cryptography (PQC) readiness for [ORGANIZATION]:
...
5. Crypto-agility: architecture changes to enable faster future algorithm transitions`,
    useWhen: "Planning migration to post-quantum cryptographic algorithms.",
    variables: ["ORGANIZATION", "INDUSTRY"]
  },
  {
    id: 238,
    title: "Supply Chain Security and SBOM Analysis Framework",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Design a supply chain security program with SBOM analysis for [ORGANIZATION]:
...
6. Metrics: SBOM coverage, vulnerability backlog, mean time to remediate supply chain issues`,
    useWhen: "Building software supply chain security capabilities with SBOM analysis.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 239,
    title: "CMMC Assessment Preparation",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Prepare for CMMC (Cybersecurity Maturity Model Certification) assessment at [ORGANIZATION]:
...
5. Timeline: gap remediation schedule aligned with contract requirements`,
    useWhen: "Preparing for CMMC certification assessment.",
    variables: ["ORGANIZATION", "LEVEL_1/LEVEL_2/LEVEL_3", "LIST", "NIST_800_171_SCORE/SPRS_SCORE", "STATUS"]
  },
  {
    id: 240,
    title: "Cybersecurity Regulatory Landscape Tracker",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Create a cybersecurity regulatory tracking framework for [ORGANIZATION] operating in [JURISDICTIONS]:
...
5. Reporting: quarterly regulatory compliance dashboard for leadership`,
    useWhen: "Managing compliance across multiple cybersecurity regulations.",
    variables: ["ORGANIZATION", "JURISDICTIONS"]
  },
  {
    id: 241,
    title: "Privacy Engineering Assessment",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Assess privacy engineering practices for [ORGANIZATION] developing [PRODUCT/SERVICE]:
...
6. Metrics: privacy request SLA compliance, DPIA completion rate, breach response time`,
    useWhen: "Evaluating privacy engineering practices and regulatory compliance.",
    variables: ["ORGANIZATION", "PRODUCT/SERVICE", "GDPR/CCPA/LGPD/PIPA/OTHER"]
  },
  {
    id: 242,
    title: "Cyber Risk Quantification for Board Reporting",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Develop a cyber risk quantification model for board-level reporting at [ORGANIZATION]:
...
5. Board presentation:
   ...
   - Recommended actions with investment requirements`,
    useWhen: "Quantifying cyber risk for board-level communication and decision-making.",
    variables: ["ORGANIZATION", "INDUSTRY", "REVENUE", "INCIDENT_HISTORY"]
  },
  {
    id: 243,
    title: "International Data Transfer Compliance Framework",
    category: "grc",
    subcategory: "Audit & Compliance",
    prompt: `Design an international data transfer compliance framework for [ORGANIZATION]:
...
6. Documentation: transfer register, TIA reports, supplementary measures evidence`,
    useWhen: "Managing compliance for international personal data transfers.",
    variables: ["ORGANIZATION", "LIST", "PERSONAL/SENSITIVE/HEALTH/FINANCIAL/EMPLOYEE", "LIST_SERVICES_AND_METHODS"]
  },
  {
    id: 244,
    ...
    prompt: `Analyze [CVE_ID] and assess its impact on our environment:
...
12. Communication: who needs to know and what do they need to do?`,
    useWhen: "Rapidly assessing new CVE impact on your environment.",
    variables: ["CVE_ID", "BRIEF_DESCRIPTION_IF_KNOWN"]
  },
  {
    id: 245,
    title: "Zero-Day Vulnerability Response Plan",
    category: "vuln-analysis",
    subcategory: "CVE Analysis",
    prompt: `Create a response plan for zero-day vulnerability [CVE_ID_OR_DESCRIPTION] affecting [PRODUCT]:
...
10. Timeline: hour-by-hour response plan for first 72 hours`,
    useWhen: "Responding to zero-day vulnerability disclosures.",
    variables: ["CVE_ID_OR_DESCRIPTION", "PRODUCT", "PASTE_ADVISORY_OR_DETAILS"]
  },
  {
    id: 246,
    title: "Vulnerability Trending Analysis",
    category: "vuln-analysis",
    subcategory: "CVE Analysis",
    prompt: `Analyze vulnerability trends for [ORGANIZATION] over the past [TIME_PERIOD]:
...
11. Forecasting: projected risk if current trends continue`,
    useWhen: "Analyzing vulnerability management program effectiveness over time.",
    variables: ["ORGANIZATION", "TIME_PERIOD", "SCANNER", "DATE_RANGE", "COUNT"]
  },
  {
    id: 247,
    title: "Exploit Prediction Analysis",
    category: "vuln-analysis",
    subcategory: "CVE Analysis",
    prompt: `Predict the exploitability of [CVE_ID] based on available information:
...
11. Recommended response urgency based on prediction`,
    useWhen: "Prioritizing vulnerabilities based on likely exploitation.",
    variables: ["CVE_ID", "PASTE_CVE_DETAILS"]
  },
  {
    id: 248,
    title: "CVE Batch Analysis for Monthly Patch Cycle",
    category: "vuln-analysis",
    subcategory: "CVE Analysis",
    prompt: `Analyze this month's CVE batch for patch prioritization:
...
6. Create the patch deployment schedule`,
    useWhen: "Monthly vulnerability review and patch cycle planning.",
    variables: ["PASTE_LIST_OF_CVE_IDS_OR_PATCH_BULLETIN", "HIGH/MEDIUM/LOW/UNKNOWN"]
  },
  {
    id: 249,
    title: "Risk-Based Patch Prioritization Model",
    category: "vuln-analysis",
    subcategory: "Patch Prioritization",
    prompt: `Design a risk-based patch prioritization model for [ORGANIZATION]:
...
7. Quarterly model calibration process`,
    useWhen: "Implementing risk-based vulnerability prioritization.",
    variables: ["ORGANIZATION", "COUNT", "TYPES", "HOW_MANY_CAN_BE_PATCHED_PER_CYCLE"]
  },
  {
    id: 250,
    title: "Emergency Patch Deployment Plan",
    category: "vuln-analysis",
    subcategory: "Patch Prioritization",
    prompt: `Create an emergency patch deployment plan for [CVE_ID] affecting [PRODUCT] across [COUNT] systems:
...
7. Fallback: if patch causes issues, exact rollback steps`,
    useWhen: "Deploying critical patches under time pressure.",
    variables: ["CVE_ID", "PRODUCT", "COUNT", "YES/NO", "INTERNET_FACING/INTERNAL/BOTH", "CRITICAL/HIGH/MEDIUM", "LAB/STAGING", "KEY_APPLICATIONS"]
  },
  {
    id: 251,
    title: "Legacy System Vulnerability Management",
    category: "vuln-analysis",
    subcategory: "Patch Prioritization",
    prompt: `Create a vulnerability management strategy for legacy systems at [ORGANIZATION]:
...
10. Executive communication on legacy risk`,
    useWhen: "Managing vulnerability risk in systems that cannot be easily patched.",
    variables: ["ORGANIZATION", "LIST_LEGACY_SYSTEMS_WITH_OS_VERSIONS_AND_BUSINESS_FUNCTION"]
  },
  {
    id: 252,
    ...
    category: "vuln-analysis",
    subcategory: "Patch Prioritization",
    prompt: `Design a patch compliance reporting framework for [ORGANIZATION]:
...
8. Benchmarking against internal and external standards`,
    useWhen: "Building patch management reporting and accountability.",
    variables: ["ORGANIZATION", "TOOL", "FRAMEWORKS", "FRAMEWORK"]
  },
  {
    id: 253,
    ...
    prompt: `Write a vulnerability assessment report for web application [APP_NAME]:
...
- Risk rating distribution chart`,
    useWhen: "Writing professional web application vulnerability assessment reports.",
    variables: ["APP_NAME", "LIST_VULNERABILITIES_WITH_DETAILS"]
  },
  {
    id: 254,
    title: "Network Vulnerability Scan Results Analysis",
    category: "vuln-analysis",
    subcategory: "Vulnerability Assessment",
    prompt: `Analyze these network vulnerability scan results from [SCANNER]:
...
10. Comparison with previous scan (if data available)`,
    useWhen: "Making scan results actionable for remediation teams.",
    variables: ["SCANNER", "PASTE_SCAN_SUMMARY_OR_KEY_FINDINGS"]
  },
  {
    id: 255,
    title: "Infrastructure Vulnerability Assessment Scope",
    category: "vuln-analysis",
    subcategory: "Vulnerability Assessment",
    prompt: `Define the scope for an infrastructure vulnerability assessment of [ORGANIZATION]:
...
11. Success criteria and quality metrics`,
    useWhen: "Scoping infrastructure vulnerability assessments.",
    variables: ["ORGANIZATION", "LIST", "RANGES", "CRITICAL_SYSTEMS_REQUIRING_CAREFUL_HANDLING"]
  },
  {
    id: 256,
    ...
    category: "vuln-analysis",
    subcategory: "Vulnerability Assessment",
    prompt: `Assess container vulnerabilities across our [CONTAINER_ENVIRONMENT]:
...
8. Continuous monitoring approach for running containers`,
    useWhen: "Assessing vulnerabilities in containerized environments.",
    variables: ["CONTAINER_ENVIRONMENT", "REGISTRY", "DOCKER/CONTAINERD/CRIO", "K8S/ECS/SWARM", "ESTIMATE"]
  },
  {
    id: 257,
    title: "API Vulnerability Assessment Checklist",
    category: "vuln-analysis",
    subcategory: "Vulnerability Assessment",
    prompt: `Create a comprehensive API vulnerability assessment checklist for [API_NAME]:
...
For each: specific test cases, tools to use, and pass/fail criteria.`,
    useWhen: "Systematically testing API security.",
    variables: ["API_NAME"]
  },
  {
    id: 258,
    title: "Vulnerability Disclosure Program Design",
    category: "vuln-analysis",
    subcategory: "Vulnerability Assessment",
    prompt: `Design a vulnerability disclosure program (VDP) or bug bounty program for [ORGANIZATION]:
...
12. Launch plan and researcher communication strategy`,
    useWhen: "Establishing external vulnerability reporting channels.",
    variables: ["ORGANIZATION", "DESCRIPTION", "LEVEL", "AMOUNT_OR_NONE_VDP_ONLY"]
  },
  {
    id: 259,
    title: "Executive Vulnerability Summary",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Write an executive vulnerability summary for [ORGANIZATION]'s [TIME_PERIOD]:
...
10. Appendix: detailed metrics for reference`,
    useWhen: "Reporting vulnerability status to non-technical leadership.",
    variables: ["ORGANIZATION", "TIME_PERIOD", "COUNT", "PERCENTAGES", "CHANGES"]
  },
  {
    id: 260,
    title: "Vulnerability Assessment Comparison Report",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Write a comparative vulnerability assessment report between scans:
...
11. Target metrics for next assessment period`,
    useWhen: "Demonstrating vulnerability management progress over time.",
    variables: ["DATE", "SCAN_ID"]
  },
  {
    id: 261,
    title: "Penetration Test Findings Remediation Tracker",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Create a remediation tracker for penetration test findings from [ENGAGEMENT_NAME]:
...
12. Dashboard view: overall progress percentage, SLA compliance`,
    useWhen: "Tracking remediation of penetration test findings.",
    variables: ["ENGAGEMENT_NAME", "LIST_FINDINGS_WITH_SEVERITY_AND_AFFECTED_SYSTEMS"]
  },
  {
    id: 262,
    title: "Vulnerability Root Cause Analysis",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Conduct a root cause analysis for recurring vulnerability [VULNERABILITY_TYPE] across [ORGANIZATION]:
...
9. Success metrics and measurement plan`,
    useWhen: "Addressing vulnerabilities that keep recurring despite patches.",
    variables: ["VULNERABILITY_TYPE", "ORGANIZATION", "COUNT", "LIST", "WHAT_WAS_DONE"]
  },
  {
    id: 263,
    ...
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Analyze the Software Bill of Materials (SBOM) for [APPLICATION_NAME] for vulnerability exposure:
...
12. Continuous monitoring strategy for the SBOM`,
    useWhen: "Analyzing software supply chain risk through SBOM.",
    variables: ["APPLICATION_NAME", "PASTE_OR_REFERENCE_LOCATION"]
  },
  {
    id: 264,
    title: "Automated Vulnerability Remediation Playbook",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Design automated vulnerability remediation playbooks for [ORGANIZATION]:
...
For each playbook: decision logic, automation platform, testing plan, and exception handling.`,
    useWhen: "Automating vulnerability remediation for common, repeatable scenarios.",
    variables: ["ORGANIZATION", "VM_SCANNER", "CSPM", "SPECIFIC_MISCONFIGURATION_TYPE", "SCA_TOOL"]
  },
  {
    id: 265,
    title: "Vulnerability Intelligence Feed Evaluation",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Evaluate vulnerability intelligence feeds and sources for [ORGANIZATION]:
...
10. Metric: time from disclosure to our awareness (goal: < [X] hours)`,
    useWhen: "Optimizing vulnerability intelligence sources and timeliness.",
    variables: ["ORGANIZATION", "LIST", "WHAT_WE_NEED_TO_KNOW_AND_HOW_FAST", "CANDIDATES"]
  },
  {
    id: 266,
    title: "Vulnerability Scan Configuration Optimization",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Optimize vulnerability scanning configuration for [SCANNER] at [ORGANIZATION]:
...
10. Quality assurance: periodic scan accuracy validation`,
    useWhen: "Improving vulnerability scanning effectiveness and efficiency.",
    variables: ["SCANNER", "ORGANIZATION", "FREQUENCY", "AUTHENTICATED/UNAUTHENTICATED", "SCOPE", "IF_ANY"]
  },
  {
    id: 267,
    title: "Application Security Testing Integration",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Design an application security testing strategy integrated into the SDLC for [ORGANIZATION]:
...
12. Metrics: vulnerabilities found per stage, fix rates, mean time to fix`,
    useWhen: "Implementing shift-left security testing.",
    variables: ["ORGANIZATION", "LANGUAGES", "PLATFORM", "GIT_PLATFORM", "TARGET_ENVIRONMENT"]
  },
  {
    id: 268,
    title: "Vulnerability Remediation SLA Framework",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Design a vulnerability remediation SLA framework for [ORGANIZATION]:
...
10. Incentive/penalty framework for teams`,
    useWhen: "Establishing vulnerability remediation expectations and accountability.",
    variables: ["ORGANIZATION", "FRAMEWORKS", "APPETITE", "ESTIMATE"]
  },
  {
    id: 269,
    title: "Configuration Vulnerability Assessment",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Conduct a configuration vulnerability assessment for [SYSTEM_TYPE: Windows Server / Linux / Network Device / Database]:
...
9. Baseline documentation for future comparison`,
    useWhen: "Assessing system configuration against security benchmarks.",
    variables: ["SYSTEM_NAME_AND_VERSION"]
  },
  {
    id: 270,
    ...
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Design a third-party component vulnerability management process for [ORGANIZATION]:
...
10. Metrics: component vulnerability density, update currency, mean time to patch`,
    useWhen: "Establishing processes for managing third-party software risk.",
    variables: ["ORGANIZATION", "COUNT", "DESCRIBE"]
  },
  {
    id: 271,
    title: "Vulnerability Simulation and Testing",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Design a vulnerability simulation exercise for [ORGANIZATION]:
...
10. Improvement actions based on results`,
    useWhen: "Testing vulnerability detection and response capabilities.",
    variables: ["ORGANIZATION", "VULNERABILITY_TYPE", "TARGET_ENVIRONMENT"]
  },
  {
    id: 272,
    title: "Mobile Application Vulnerability Report",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Write a mobile application vulnerability report for [APP_NAME] on [ANDROID/IOS]:
...
5. Appendix: tool output, configuration details`,
    useWhen: "Writing mobile application security assessment reports.",
    variables: ["APP_NAME", "ANDROID/IOS", "LIST_FINDINGS"]
  },
  {
    id: 273,
    title: "Vulnerability Management Program Maturity Assessment",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Assess the maturity of [ORGANIZATION]'s vulnerability management program:
...
10. Rate each dimension 1-5, identify top 3 improvement priorities, create 12-month roadmap`,
    useWhen: "Evaluating and improving vulnerability management program maturity.",
    variables: ["ORGANIZATION", "DESCRIBE_CURRENT_VM_PRACTICES"]
  },
  {
    id: 274,
    title: "IoT Device Vulnerability Assessment",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Assess vulnerabilities in IoT devices at [ORGANIZATION]:
...
11. Risk acceptance criteria for unpatchable devices`,
    useWhen: "Assessing IoT device security in enterprise environments.",
    variables: ["ORGANIZATION", "ESTIMATE", "SEGMENTED/FLAT"]
  },
  {
    id: 275,
    ...
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Conduct a GraphQL API security assessment for [APPLICATION] during an authorized test:
...
6. Reporting: findings mapped to OWASP GraphQL security guidelines`,
    useWhen: "Testing GraphQL API security during authorized assessments.",
    variables: ["APPLICATION", "URL", "INTROSPECTION_ENABLED/SCHEMA_FILE/BLIND"]
  },
  {
    id: 276,
    title: "Firmware Vulnerability Analysis",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Conduct firmware vulnerability analysis for [DEVICE_TYPE] at [ORGANIZATION]:
...
6. Reporting: vulnerability findings with CVSS scoring and remediation recommendations`,
    useWhen: "Analyzing firmware security of IoT, embedded, or network devices.",
    variables: ["DEVICE_TYPE", "ORGANIZATION", "VENDOR", "MODEL", "VERSION", "DOWNLOADED/EXTRACTED_FROM_DEVICE"]
  },
  {
    id: 277,
    ...
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Assess hardware-level vulnerability exposure for [ORGANIZATION]:
...
5. Prioritized remediation: firmware updates, microcode patches, configuration changes`,
    useWhen: "Assessing hardware-level vulnerabilities across infrastructure.",
    variables: ["ORGANIZATION", "LIST"]
  },
  {
    id: 278,
    title: "Vulnerability Chaining Analysis",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Conduct vulnerability chaining analysis for [ORGANIZATION] infrastructure:
...
5. Reporting: attack path diagrams, chain risk scores, prioritized remediation that maximizes risk reduction`,
    useWhen: "Analyzing how individual vulnerabilities combine into high-impact attack paths.",
    variables: ["ORGANIZATION", "PASTE_OR_DESCRIBE_FINDINGS", "DESCRIBE_SEGMENTATION", "LIST_CRITICAL_ASSETS"]
  },
  {
    id: 279,
    ...
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Assess browser extension security for [ORGANIZATION]:
...
5. Remediation: unauthorized extension removal, policy enforcement, approved alternatives`,
    useWhen: "Auditing browser extension security across the organization.",
    variables: ["ORGANIZATION", "LIST", "CHROME/EDGE/FIREFOX", "MDM/GPO/UNMANAGED"]
  },
  {
    id: 280,
    title: "Vulnerability Correlation with Threat Intelligence",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Correlate vulnerability data with threat intelligence for [ORGANIZATION]:
...
5. Reporting: threat-informed vulnerability priority list with intelligence context`,
    useWhen: "Enhancing vulnerability prioritization with threat intelligence context.",
    variables: ["ORGANIZATION", "TI_SOURCES", "INDUSTRY", "SPECIFIC_THREAT_ACTORS"]
  },
  {
    id: 281,
    title: "Vulnerability Assessment for CI/CD Pipelines",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Assess vulnerability exposure in CI/CD pipeline infrastructure at [ORGANIZATION]:
...
6. Findings with remediation recommendations and implementation priority`,
    useWhen: "Assessing security vulnerabilities in CI/CD pipeline infrastructure.",
    variables: ["ORGANIZATION", "GITHUB/GITLAB/BITBUCKET/AZURE_DEVOPS", "JENKINS/GITHUB_ACTIONS/GITLAB_CI/CIRCLECI/AZURE_PIPELINES", "LIST"]
  },
  {
    id: 282,
    title: "Network Protocol Vulnerability Assessment",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Conduct a network protocol vulnerability assessment for [ORGANIZATION]:
...
5. Remediation priority: protocol upgrade plan with compatibility impact assessment`,
    useWhen: "Identifying vulnerable network protocols across the environment.",
    variables: ["ORGANIZATION", "LIST"]
  },
  {
    id: 283,
    title: "Vulnerability Impact Simulation",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Design and execute vulnerability impact simulations for [ORGANIZATION]:
...
5. Results: impact report for each scenario, ROI justification for remediation investment`,
    useWhen: "Quantifying the real-world impact of vulnerabilities for prioritization.",
    variables: ["ORGANIZATION", "LIST_CRITICAL_VULNERABILITIES_TO_SIMULATE", "RELATED_VULNS"]
  },
  {
    id: 284,
    title: "API Security Vulnerability Deep Dive",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Conduct a deep-dive API vulnerability assessment for [APPLICATION_NAME]:
...
11. Additional: JWT vulnerabilities (algorithm confusion, key exposure, claim tampering)`,
    useWhen: "Conducting thorough API security assessments against current standards.",
    variables: ["APPLICATION_NAME", "OPENAPI_SPEC/SWAGGER/MANUAL", "API_KEY/OAUTH2/JWT/BASIC/NONE", "COUNT"]
  },
  {
    id: 285,
    title: "Vulnerability Management Automation Framework",
    category: "vuln-analysis",
    subcategory: "Vulnerability Reporting",
    prompt: `Design a vulnerability management automation framework for [ORGANIZATION]:
...
6. Integration architecture: API connections between all tools with data flow diagram`,
    useWhen: "Automating vulnerability management lifecycle processes.",
    variables: ["ORGANIZATION", "TOOL"]
  },
  {
    id: 286,
    title: "Prompt Injection Vulnerability Assessment",
    category: "ai-security",
    subcategory: "LLM Vulnerability Testing",
    prompt: `Design a prompt injection vulnerability assessment for [AI_APPLICATION]:
...
5. Automated test harness design for regression testing`,
    useWhen: "Testing AI applications for prompt injection vulnerabilities.",
    variables: ["AI_APPLICATION", "OPENAI/ANTHROPIC/GOOGLE/OPEN_SOURCE", "CHATBOT/AGENT/RAG_SYSTEM/CODE_ASSISTANT", "DIRECT/PREPROCESSED/FILTERED", "AVAILABLE/UNKNOWN"]
  },
  {
    id: 287,
    title: "LLM Jailbreak Detection System Design",
    category: "ai-security",
    subcategory: "LLM Vulnerability Testing",
    prompt: `Design a jailbreak detection system for [AI_APPLICATION]:
...
7. Continuous update process as new jailbreak techniques emerge`,
    useWhen: "Building defenses against LLM jailbreak attacks.",
    variables: ["AI_APPLICATION", "DESCRIBE_EXISTING_SAFETY_MEASURES"]
  },
  {
    id: 288,
    title: "AI Red Team Exercise Design",
    category: "ai-security",
    subcategory: "LLM Vulnerability Testing",
    prompt: `Design an AI red team exercise for [AI_SYSTEM]:
...
7. Remediation guidance per vulnerability category`,
    useWhen: "Conducting structured security testing of AI systems.",
    variables: ["AI_SYSTEM", "WHAT_THE_AI_DOES", "WHO_USES_IT", "WHAT_DATA_CAN_IT_ACCESS", "WHAT_ACTIONS_CAN_IT_TAKE", "CURRENT_GUARDRAILS"]
  },
  {
    id: 289,
    title: "LLM Output Validation Framework",
    category: "ai-security",
    subcategory: "LLM Vulnerability Testing",
    prompt: `Design an output validation framework for [AI_APPLICATION]:
...
6. Feedback loop: improving validation based on detected issues`,
    useWhen: "Building safety layers around AI system outputs.",
    variables: ["AI_APPLICATION", "TEXT/CODE/SQL/API_CALLS/STRUCTURED_DATA", "HIGH/MEDIUM/LOW", "WHAT_HAPPENS_WITH_AI_OUTPUT"]
  },
  {
    id: 290,
    title: "AI Model Security Testing Checklist",
    category: "ai-security",
    subcategory: "LLM Vulnerability Testing",
    prompt: `Create a security testing checklist for deploying [MODEL_NAME] in production:
...
12. Incident response: AI-specific incident procedures`,
    useWhen: "Security review before deploying AI models to production.",
    variables: ["MODEL_NAME", "SELF_HOSTED/API/CLOUD_ML_SERVICE", "HOW_DOES_IT_CONNECT_TO_OTHER_SYSTEMS"]
  },
  {
    id: 291,
    ...
    category: "ai-security",
    subcategory: "LLM Vulnerability Testing",
    prompt: `Assess training data poisoning risks for [AI_SYSTEM]:
...
6. Monitoring: detecting model drift due to poisoning post-deployment`,
    useWhen: "Evaluating and mitigating training data poisoning risks.",
    variables: ["AI_SYSTEM", "LIST_SOURCES", "PREPROCESSING_STEPS", "HOW_OFTEN", "SIZE"]
  },
  {
    id: 292,
    title: "AI Model Supply Chain Audit",
    category: "ai-security",
    subcategory: "AI Supply Chain Security",
    prompt: `Audit the supply chain for AI models used by [ORGANIZATION]:
...
6. Risk rating per model and remediation recommendations`,
    useWhen: "Auditing AI model supply chain risks.",
    variables: ["ORGANIZATION", "LIST_AI_MODELS_AND_THEIR_SOURCES"]
  },
  {
    id: 293,
    title: "AI/ML Pipeline Security Architecture",
    category: "ai-security",
    subcategory: "AI Supply Chain Security",
    prompt: `Design a secure AI/ML pipeline for [ORGANIZATION]:
...
7. End-to-end: access control, audit logging, incident response`,
    useWhen: "Designing secure MLOps pipelines.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 294,
    title: "Open Source LLM Security Evaluation",
    category: "ai-security",
    subcategory: "AI Supply Chain Security",
    prompt: `Evaluate the security of open source LLM [MODEL_NAME] before organizational deployment:
...
10. Risk acceptance documentation`,
    useWhen: "Evaluating open source LLMs for organizational use.",
    variables: ["MODEL_NAME", "HUGGING_FACE/GITHUB/OTHER", "SIZE", "USE_CASE"]
  },
  {
    id: 295,
    title: "AI Vendor Security Assessment",
    category: "ai-security",
    subcategory: "AI Supply Chain Security",
    prompt: `Assess the security posture of AI vendor [VENDOR_NAME] providing [SERVICE_TYPE]:
...
6. Contractual:
   ...
   - Exit strategy and data portability`,
    useWhen: "Evaluating AI service providers for security risks.",
    variables: ["VENDOR_NAME", "SERVICE_TYPE"]
  },
  {
    id: 296,
    title: "AI Dependency Vulnerability Scanning",
    category: "ai-security",
    subcategory: "AI Supply Chain Security",
    prompt: `Design an AI-specific dependency vulnerability scanning process for [ORGANIZATION]:
...
7. Emergency response plan for critical AI framework CVEs`,
    useWhen: "Securing AI software dependencies.",
    variables: ["ORGANIZATION", "NVIDIA_DRIVERS/CUDA/DOCKER/K8S"]
  },
  {
    id: 297,
    title: "AI-Powered Anomaly Detection System Design",
    category: "ai-security",
    subcategory: "AI-Powered Threat Detection",
    prompt: `Design an AI-powered anomaly detection system for [SECURITY_DOMAIN: network traffic / user behavior / application logs]:
...
10. Explainability: how to help analysts understand why an alert fired`,
    useWhen: "Building ML-based anomaly detection for security operations.",
    variables: ["LIST", "EVENTS_PER_SECOND", "RULE_BASED/SIGNATURE/NONE"]
  },
  {
    id: 298,
    title: "AI-Assisted Threat Hunting Queries",
    category: "ai-security",
    subcategory: "AI-Powered Threat Detection",
    prompt: `Generate AI-assisted threat hunting hypotheses and queries for [ENVIRONMENT]:
...
5. How to use LLMs to summarize and prioritize hunting results`,
    useWhen: "Enhancing threat hunting with AI-driven analysis.",
    variables: ["ENVIRONMENT", "EDR_PRODUCT", "NDR/FLOW/PCAP", "IAM/DIRECTORY", "CLOUD_PROVIDER"]
  },
  {
    id: 299,
    title: "LLM-Based Log Analysis Assistant",
    category: "ai-security",
    subcategory: "AI-Powered Threat Detection",
    prompt: `Design a LLM-based log analysis assistant for [SOC_TEAM]:
...
6. Accuracy measurement and continuous improvement`,
    useWhen: "Building AI-powered tools to assist security analysts.",
    variables: ["SOC_TEAM"]
  },
  {
    id: 300,
    title: "AI-Enhanced Phishing Detection",
    category: "ai-security",
    subcategory: "AI-Powered Threat Detection",
    prompt: `Design an AI-enhanced phishing detection system for [ORGANIZATION]:
...
7. Handling zero-day phishing (previously unseen campaigns)`,
    useWhen: "Building or improving AI-driven phishing detection.",
    variables: ["ORGANIZATION", "EXISTING_TOOLS", "DAILY_COUNT"]
  },
  {
    id: 301,
    title: "AI for Vulnerability Prioritization",
    category: "ai-security",
    subcategory: "AI-Powered Threat Detection",
    prompt: `Design an AI-powered vulnerability prioritization system for [ORGANIZATION]:
...
8. Comparison metrics: AI-prioritized vs. CVSS-only prioritization`,
    useWhen: "Moving beyond CVSS to intelligent vulnerability prioritization.",
    variables: ["ORGANIZATION", "TOOL", "COUNT", "CVSS_ONLY/BASIC_RISK_BASED"]
  },
  {
    id: 302,
    title: "AI Agent Security Architecture",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design a security architecture for an AI agent [AGENT_NAME] that can [DESCRIBE_CAPABILITIES]:
...
6. Isolation:
   ...
   - Data boundary enforcement`,
    useWhen: "Designing security controls for AI agents with tool access.",
    variables: ["AGENT_NAME", "DESCRIBE_CAPABILITIES", "LIST_DATA_SOURCES", "LIST_ACTIONS", "DIRECT_CHAT/AUTOMATED/HUMAN_IN_LOOP"]
  },
  {
    id: 303,
    title: "AI Agent Permissions and Guardrails Framework",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Create a permissions and guardrails framework for AI agents at [ORGANIZATION]:
...
7. Incident scenarios: what to do when an agent acts unexpectedly`,
    useWhen: "Establishing governance for AI agents across the organization.",
    variables: ["ORGANIZATION", "LIST_AGENTS_AND_THEIR_FUNCTIONS"]
  },
  {
    id: 304,
    title: "AI Agent Monitoring and Observability",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design a monitoring and observability system for AI agents in production at [ORGANIZATION]:
...
7. Incident response playbook for agent-specific incidents`,
    useWhen: "Building observability for AI agents in production.",
    variables: ["ORGANIZATION", "LIST_AGENTS_WITH_FUNCTIONS"]
  },
  {
    id: 305,
    title: "Secure Tool-Use Implementation for AI Agents",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design secure tool-use implementation for an AI agent that needs to use [LIST_TOOLS]:
...
7. Failure modes:
   ...
   - Human escalation triggers`,
    useWhen: "Implementing secure tool integration for AI agents.",
    variables: ["LIST_TOOLS", "DESCRIBE_EACH_TOOL_AND_ITS_CAPABILITIES"]
  },
  {
    id: 306,
    ...
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Create an incident response playbook for AI agent-related security incidents:
...
For each: severity classification, escalation path, communication template.`,
    useWhen: "Building IR capabilities for AI agent security incidents.",
    variables: ["LIST_PRODUCTION_AGENTS"]
  },
  {
    id: 307,
    title: "AI Agent Penetration Testing Methodology",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design a penetration testing methodology for AI agents deployed at [ORGANIZATION]:
...
7. Report template with AI-specific severity ratings`,
    useWhen: "Penetration testing AI agent deployments.",
    variables: ["ORGANIZATION", "AGENT_NAME", "WHAT_IT_CAN_DO", "EXISTING_DEFENSES"]
  },
  {
    id: 308,
    title: "Responsible AI Security Policy",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Draft a Responsible AI Security Policy for [ORGANIZATION]:
...
12. Review and update cadence`,
    useWhen: "Establishing organizational AI security governance.",
    variables: ["ORGANIZATION", "LIST", "YES/NO"]
  },
  {
    id: 309,
    ...
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Assess [ORGANIZATION]'s AI systems against EU AI Act requirements:
...
6. Vendor compliance requirements`,
    useWhen: "Assessing AI system compliance with the EU AI Act.",
    variables: ["ORGANIZATION", "LIST_AI_SYSTEMS_WITH_DESCRIPTIONS"]
  },
  {
    id: 310,
    title: "AI Agent Data Loss Prevention",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design a data loss prevention strategy for AI agents at [ORGANIZATION]:
...
6. Implementation: tool recommendations and architecture`,
    useWhen: "Preventing data loss through AI agent channels.",
    variables: ["ORGANIZATION", "LIST", "CATEGORIES", "TYPES_AND_REGULATIONS"]
  },
  {
    id: 311,
    ...
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design security architecture for a multi-agent system at [ORGANIZATION]:
...
7. Incident response for multi-agent compromise scenarios`,
    useWhen: "Securing multi-agent AI architectures.",
    variables: ["ORGANIZATION", "NUMBER", "LIST_WITH_CAPABILITIES", "HIERARCHICAL/MESH/PIPELINE", "FRAMEWORK"]
  },
  {
    id: 312,
    title: "AI Model Access Control Design",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design an access control system for AI models at [ORGANIZATION]:
...
6. Integration with existing IAM ([IAM_PLATFORM])`,
    useWhen: "Implementing fine-grained access control for AI systems.",
    variables: ["ORGANIZATION", "LIST_MODELS_WITH_CAPABILITIES_AND_DATA_ACCESS", "IAM_PLATFORM"]
  },
  {
    id: 313,
    ...
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design defenses against adversarial machine learning attacks for [ML_SYSTEM]:
...
6. Monitoring: detecting adversarial attacks in production`,
    useWhen: "Hardening ML systems against adversarial attacks.",
    variables: ["ML_SYSTEM", "CLASSIFICATION/DETECTION/NLP/GENERATION", "IMAGES/TEXT/TABULAR/MULTI_MODAL", "HIGH/MEDIUM/LOW"]
  },
  {
    id: 314,
    ...
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Conduct a threat model for AI system [SYSTEM_NAME]:
...
For each threat: risk rating, existing mitigations, recommended additional controls.`,
    useWhen: "Systematic threat modeling for AI systems.",
    variables: ["SYSTEM_NAME", "WHAT_IT_DOES", "COMPONENTS_AND_DATA_FLOW", "WHO_INTERACTS_WITH_IT", "WHAT_DATA_IT_PROCESSES"]
  },
  {
    id: 315,
    title: "AI Safety Testing in CI/CD Pipeline",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design AI safety testing integration into the CI/CD pipeline for [AI_APPLICATION]:
...
6. Continuous:
   ...
   - User feedback integration`,
    useWhen: "Integrating AI security testing into development pipelines.",
    variables: ["AI_APPLICATION", "CI_CD_PLATFORM"]
  },
  {
    id: 316,
    title: "RAG System Security Assessment",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Assess the security of a Retrieval-Augmented Generation (RAG) system [SYSTEM_NAME]:
...
6. Testing plan: specific test cases for each risk area`,
    useWhen: "Evaluating security of RAG-based AI systems.",
    variables: ["SYSTEM_NAME", "SOLUTION", "MODEL", "LIST", "WHO_USES_IT"]
  },
  {
    id: 317,
    title: "AI Governance Framework for Security Teams",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Create an AI governance framework for the security team at [ORGANIZATION]:
...
7. Continuous improvement:
   ...
   - Deprecation criteria`,
    useWhen: "Governing the security team\'s own use of AI tools.",
    variables: ["ORGANIZATION", "LIST_AI_TOOLS_USED_BY_SECURITY_TEAM"]
  },
  {
    id: 318,
    title: "Agentic Workflow Security Patterns",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Document security patterns for common agentic AI workflows at [ORGANIZATION]:
...
For each pattern: implementation checklist, testing requirements, and monitoring approach.`,
    useWhen: "Implementing standard security patterns for AI agent capabilities.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 319,
    title: "AI Compliance and Audit Trail Design",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design an AI audit trail system for [ORGANIZATION] to meet compliance and investigation needs:
...
5. Compliance mapping: how this meets each regulation's requirements`,
    useWhen: "Building compliant audit trails for AI systems.",
    variables: ["ORGANIZATION", "EU_AI_ACT/NIST_AI_RMF/INDUSTRY_SPECIFIC", "LIST", "PERIOD"]
  },
  {
    id: 320,
    title: "AI Incident Classification and Response Matrix",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Create an AI-specific incident classification and response matrix for [ORGANIZATION]:
...
For each category: detection method, containment steps, investigation procedures, remediation, communication requirements.`,
    useWhen: "Classifying and responding to AI-specific security incidents.",
    variables: ["ORGANIZATION"]
  },
  {
    id: 321,
    title: "Prompt Engineering Security Guide",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Create a prompt engineering security guide for developers at [ORGANIZATION]:
...
7. Security review checklist for new prompts`,
    useWhen: "Training developers on secure AI application development.",
    variables: ["ORGANIZATION", "EXAMPLES"]
  },
  {
    id: 322,
    title: "AI Model Versioning and Rollback Security",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Design a secure model versioning and rollback system for [ORGANIZATION]:
...
5. Audit:
   ...
   - Compliance evidence for model lifecycle management`,
    useWhen: "Managing AI model versions with security controls.",
    variables: ["ORGANIZATION", "LIST_SYSTEMS_WITH_UPDATE_FREQUENCY"]
  },
  {
    id: 323,
    title: "AI Security Maturity Model",
    category: "ai-security",
    subcategory: "Securing AI Agents in Production",
    prompt: `Assess AI security maturity for [ORGANIZATION] across these dimensions:
...
For each dimension: current level assessment, evidence, target level, and improvement roadmap.`,
    useWhen: "Assessing and benchmarking organizational AI security maturity.",
    variables: ["ORGANIZATION"]
  },
];
